...
#### 🧩 TL;DR
本文提出SpikeVPR，一种结合事件相机和脉冲神经网络的生物启发式视觉地点识别方法，在保持与最先进深度网络相当性能的同时，实现了50倍参数减少和30-250倍能耗降低，为动态环境下的高效VPR提供了新途径。

---

#### 📘 Detailed Summary
**Motivation:** 传统深度网络在动态真实世界条件下的视觉地点识别存在高计算和能耗限制，难以满足自主机器人的实时部署需求，需要开发更高效且鲁棒的解决方案。

**Method:** SpikeVPR采用事件相机与脉冲神经网络相结合的生物启发式方法，通过端到端的替代梯度学习进行训练，并引入EventDilation增强策略来提升对速度和时序变化的鲁棒性，从少量样本生成紧凑且不变的地点描述符。

**Result:** 在Brisbane-Event-VPR和NSAVP两个挑战性基准测试中，SpikeVPR实现了与最先进深度网络相当的性能，同时参数数量减少50倍，能耗降低30-250倍，能够在移动和神经形态平台上实时部署。

**Conclusion:** 脉冲编码为复杂变化环境中的鲁棒视觉地点识别提供了高效途径，生物启发式方法在保持性能的同时显著降低了计算和能耗需求，为自主机器人的实时导航系统开辟了新方向。
...
### [2] [DSERT-RoLL: Robust Multi-Modal Perception for Diverse Driving Conditions with Stereo Event-RGB-Thermal Cameras, 4D Radar, and Dual-LiDAR](https://arxiv.org/abs/2604.03685)
*Hoonhee Cho, Jae-Young Kang, Yuhwan Jeong, Yunseo Yang, Wonyoung Lee, Youngho Kim, Kuk-Jin Yoon*

#### 🧩 TL;DR
本文提出了DSERT-RoLL多模态驾驶数据集，集成了立体事件相机、RGB相机、热成像相机、4D雷达和双激光雷达，并建立了统一的2D/3D基准测试，以支持跨传感器组合的系统性研究和公平比较。

---

#### 📘 Detailed Summary
**Motivation:** 当前自动驾驶研究中，事件相机和4D雷达等新型传感器面临数据稀缺问题，且缺乏跨传感器组合的系统性比较框架。现有数据集难以支持在不同天气和光照条件下对多种传感器进行公平的性能评估和融合策略研究。

**Method:** 该研究构建了DSERT-RoLL多模态驾驶数据集，包含立体事件相机、RGB相机、热成像相机、4D雷达和双激光雷达的同步数据，并提供了精确的2D/3D边界框标注、轨迹ID和车辆里程计信息。此外，提出了一个融合框架，将不同传感器的特征线索整合到统一的特征空间中，以提升检测鲁棒性。

**Result:** 研究建立了统一的3D和2D基准测试，支持传感器家族内和跨家族的公平比较。报告了代表性单模态和多模态方法的基线性能，并提供了促进不同融合策略和传感器组合研究的实验协议。该数据集支持在多样天气和光照条件下的系统评估。

**Conclusion:** DSERT-RoLL数据集填补了新型传感器数据稀缺的空白，为自动驾驶感知研究提供了全面的多模态基准平台。提出的融合框架展示了整合异构传感器特征以提升环境感知鲁棒性的潜力，为未来传感器融合算法的发展奠定了基础。
...
### [3] [E-VLA: Event-Augmented Vision-Language-Action Model for Dark and Blurred Scenes](https://arxiv.org/abs/2604.04834)
*Jiajun Zhai, Hao Shi, Shangwei Guo, Kailun Yang, Kaiwei Wang*

#### 🧩 TL;DR
本文提出E-VLA框架，通过将事件相机数据直接融入视觉-语言-动作模型，显著提升了机器人在低光照、运动模糊等恶劣感知条件下的操作鲁棒性，无需从事件流重建图像。

---

#### 📘 Detailed Summary
**Motivation:** 传统的视觉-语言-动作模型在开放操作任务中表现良好，但其感知能力在极端低光照、运动模糊和黑裁剪等感知阶段退化条件下变得脆弱，这限制了机器人在现实世界复杂环境中的可靠部署。

**Method:** E-VLA框架直接利用事件流中的运动和结构线索来保持语义感知和感知-动作一致性，而非从事件重建图像；作者构建了包含DAVIS346事件相机的开源遥操作平台，收集了真实世界同步RGB-事件-动作操作数据集，并提出了轻量级、与预训练模型兼容的事件集成策略，研究了事件窗口化和融合方法以实现稳定部署。

**Result:** 实验表明，即使是简单的无参数融合方法（将累积事件图叠加到RGB图像上）也能显著提升黑暗和模糊场景下的鲁棒性：在20勒克斯光照下的Pick-Place任务中，成功率从0%（仅图像）提升至60%（叠加融合）和90%（事件适配器）；在严重运动模糊条件下，Pick-Place从0%提升至20-25%，Sorting任务从5%提升至32.5%。

**Conclusion:** E-VLA提供了系统性证据，表明事件驱动感知可以有效集成到VLA模型中，为实现超越传统帧基成像的鲁棒具身智能指明了方向；该研究强调了事件相机在恶劣感知条件下保持感知-动作一致性的潜力，为机器人感知系统的鲁棒性改进提供了新途径。
...
### [4] [YANA: Bridging the Neuromorphic Simulation-to-Hardware Gap](https://arxiv.org/abs/2604.03432)
...
#### 🧩 TL;DR
本文提出了YANA，一种基于FPGA的数字脉冲神经网络加速器，旨在弥合神经形态计算中的仿真与硬件之间的鸿沟。该框架通过可访问的硬件和软件解决方案，支持任意拓扑的SNN，并充分利用时空稀疏性实现高效事件驱动处理。

---

#### 📘 Detailed Summary
**Motivation:** 神经形态硬件有限可用性造成了显著的仿真与硬件之间的鸿沟，这阻碍了算法创新、软硬件协同设计以及成熟开源生态系统的发展。传统人工神经网络在处理时间稀疏数据流和严格功耗约束应用时存在局限性，而现有神经形态计算平台的可访问性不足限制了该领域的研究进展。

**Method:** YANA采用基于FPGA的数字SNN加速器架构，实现五阶段事件驱动处理流水线，完全利用时空稀疏性并支持通过点对点神经元连接的任意SNN拓扑。该架构包含输入预处理方案，确保每个周期稳定处理一个事件而无缓冲区溢出风险，并使用查找表实现硬件高效的泄漏计算事件驱动神经元更新。

**Result:** 在Spiking Heidelberg Digits数据集上的实验展示了YANA的稀疏性利用能力，推理时间随空间和时间稀疏度水平呈现近似线性缩放。在AMD Kria KR260平台上，单个YANA核心使用740个LUT、918个寄存器、7个BRAM和24个URAM，支持最多2¹⁷个突触和2¹⁰个神经元。

**Conclusion:** YANA框架作为开源项目发布，提供了从训练、优化到部署SNN的端到端解决方案，并通过神经形态中间表示与现有神经形态计算工具集成。这项工作显著降低了神经形态计算的研究门槛，促进了算法创新和软硬件协同设计，为构建更成熟的神经形态计算生态系统奠定了基础。
...
### [5] [Regime Mapping of Oscillatory States in Balanced Spiking Networks with Multiple Time Scales](https://arxiv.org/abs/2604.04770)
*Tsung-Han Kuo, Tzu-Chia Tung*

#### 🧩 TL;DR
该研究系统性地绘制了平衡尖峰网络中突触后衰减时间常数、传导延迟和可塑性速率如何共同塑造振荡状态的参数空间图谱，揭示了可塑性扩展振荡区域并增强节律相干性的机制。

---

#### 📘 Detailed Summary
**Motivation:** 平衡尖峰网络可以在静默、异步不规则和振荡状态之间转换，但其突触时间尺度、传导延迟和可塑性速率等参数的联合结构尚未得到完整表征，这限制了我们对网络状态转换机制的理解和操作点选择。

**Method:** 研究采用Brian2模拟平台，在(τs, d, λp)参数空间中系统扫描，结合粗粒化的Hopf参考边界分析，构建了状态转换图谱和频谱显著度地形图，并通过代表性控制实验验证局部节律形成机制。

**Result:** 参数图谱显示增加可塑性速率λp会向较短τs和中长延迟方向扩展振荡区域，显著度图谱识别出最强节律相干性的参数区域；控制实验表明STDP冻结会削弱节律相干性，而延迟抖动在保持平均发放率基本不变的情况下增强节律相干性。

**Conclusion:** 该研究为平衡网络设置中的操作点选择、同步性调制研究和未来生物基础建模提供了实用参考，揭示了参数相互作用如何共同塑造网络振荡状态，并建立了全局参数景观与局部节律形成机制之间的连接。
