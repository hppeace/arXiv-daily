...
#### 📘 Detailed Summary
**Motivation:** 事件相机因其高动态范围和低延迟特性，在挑战性环境中具有鲁棒深度估计的潜力，但现有方法受限于小规模标注数据集，限制了其在真实场景中的泛化能力。

**Method:** 研究引入了EvtSlowTV大规模事件相机数据集，构建了自监督学习框架以利用原始事件流的高动态范围潜力，该方法无需基于帧的标注并保持了事件数据的异步特性。

**Result:** EvtSlowTV数据集比现有事件数据集大一个数量级，包含各种环境条件和运动场景，训练使用该数据集能显著提升模型在复杂场景和运动中的泛化能力。

**Conclusion:** 该研究证明了大规模无约束自然场景事件数据对于深度估计的重要性，自监督学习框架能够有效利用事件相机的高动态范围特性，为事件视觉在真实世界应用中的发展提供了重要基础。
...
### [2] [A Lightweight 3D-CNN for Event-Based Human Action Recognition with Privacy-Preserving Potential](https://arxiv.org/abs/2511.03665)
*Mehdi Sefidgar Dilmaghani, Francis Fowley, Peter Corcoran*

#### 🧩 TL;DR
本文提出了一种轻量级三维卷积神经网络，利用事件视觉数据进行人类活动识别，在保护隐私的同时实现了94.17%的准确率，优于现有3D-CNN基准模型。

---

#### 📘 Detailed Summary
**Motivation:** 传统基于帧的摄像头在人类监控系统中会捕获可识别的个人信息，存在隐私保护挑战，而事件相机仅记录像素强度变化，提供了一种固有的隐私保护感知模式，但需要有效的深度学习方法来实现准确的活动识别。

**Method:** 采用轻量级三维卷积神经网络有效建模空间和时间动态，同时保持紧凑设计以适应边缘部署；为解决类别不平衡和增强泛化能力，使用了带类别重新加权的焦点损失和针对性数据增强策略。

**Result:** 在丰田智能家居和ETRI数据集组合上的实验结果显示，模型F1分数达到0.9415，整体准确率为94.17%，比C3D、ResNet3D和MC3_18等基准3D-CNN架构性能提升高达3%。

**Conclusion:** 研究结果表明基于事件的深度学习方法具有开发准确、高效且隐私感知的人类动作识别系统的潜力，特别适合现实世界的边缘应用场景，为隐私保护监控系统提供了有前景的技术路径。
