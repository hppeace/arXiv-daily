...
### [1] [IE2Video: Adapting Pretrained Diffusion Models for Event-Based Video Reconstruction](https://arxiv.org/abs/2512.05240)
*Dmitrii Torbunov, Onur Okuducu, Yi Huang, Odera Dim, Rebecca Coles, Yonggang Cui, Yihui Ren*

#### 🧩 TL;DR
本文提出了一种混合捕获范式，通过结合稀疏RGB关键帧和连续事件流来重建完整RGB视频，以解决事件相机低功耗但输出非标准视频格式的问题。研究引入了IE2Video任务，并探索了自回归和扩散模型两种架构策略，其中基于扩散的方法在感知质量上显著优于基线。

---

#### 📘 Detailed Summary
**Motivation:** 传统RGB相机在连续视频监控中面临高功耗限制，而事件相机虽然功耗低但产生异步事件流而非标准RGB视频。本研究旨在解决这一矛盾，提出混合捕获范式以在降低捕获功耗的同时保持标准视频输出，满足下游应用需求。

**Method:** 研究引入了图像和事件到视频（IE2Video）任务，探索了两种架构策略：为RGB生成调整自回归模型（HyperE2VID），以及通过学习的编码器和低秩适应将事件表示注入预训练的文本到视频扩散模型（LTX）。该方法结合稀疏RGB关键帧和连续事件流进行离线视频重建。

**Result:** 实验表明，基于扩散的方法在感知质量上比自回归基线提高了33%（LPIPS得分0.283 vs 0.422）。研究在三个事件相机数据集（BS-ERGB、HS-ERGB远/近）和不同序列长度（32-128帧）上验证了该方法，展示了在未见捕获配置上的强大跨数据集泛化能力。

**Conclusion:** 该研究证明了混合捕获范式在降低功耗同时保持视频质量的可行性，扩散模型在事件到视频转换任务中表现出优越性能。这项工作为低功耗视频监控系统提供了新方向，并展示了预训练扩散模型在跨模态视频重建任务中的适应潜力。
...
#### 🧩 TL;DR
本文提出了一种名为NeuromorphicRx的新型节能脉冲神经网络接收器，用于5G-NR OFDM系统，该接收器能够以显著降低的能耗实现与传统接收器相当的性能。

---

#### 📘 Detailed Summary
**Motivation:** 本研究旨在解决5G-NR系统中传统接收器在能效方面的局限性，通过开发基于脉冲神经网络的节能接收器来替代传统的信道估计、均衡和符号解映射模块，同时保持或提升系统性能。

**Method:** 该方法提出了一种深度卷积脉冲神经网络架构，采用脉冲元素级残差连接，并利用领域知识设计具有脉冲编码的输入表示。研究采用了SNN与人工神经网络的混合架构以获得软输出，并应用代理梯度下降进行训练，同时通过量化感知训练增强鲁棒性和跨场景泛化能力。

**Result:** 广泛的数值仿真表明，NeuromorphicRx在块错误率性能上相比传统5G-NR接收器获得显著提升，同时与基于ANN的对应方案性能相当，但能耗降低了7.6倍。研究还进行了详细的消融实验以验证对5G-NR信号的可解释性。

**Conclusion:** 该研究证明了脉冲神经网络在无线通信接收器设计中的可行性和优势，为开发高能效的下一代通信系统提供了新途径。混合架构和量化感知训练的结合为实现实际部署中的鲁棒性和泛化能力提供了有效解决方案。
...
#### 🧩 TL;DR
本文提出时空可分离网络（STSep），通过解耦空间和时间分支来优化脉冲神经网络在视频理解中的时空建模能力，揭示了膜电位传播在复杂时序任务中的实际贡献有限，并提供了更有效的时空表示学习框架。

---

#### 📘 Detailed Summary
**Motivation:** 现有研究普遍认为脉冲神经网络的膜电位传播是其核心时序建模机制，但缺乏对其在复杂时序任务中实际贡献的系统分析，需要量化膜电位传播在时空表示学习中的具体作用并探索更有效的时空建模方法。

**Method:** 首先设计非状态模型逐步移除膜电位传播以量化其分阶段作用，然后基于时空资源竞争理论提出时空可分离网络，将残差块解耦为独立的空间分支和时间分支，空间分支专注于语义提取，时间分支通过显式时间差异捕获运动信息。

**Result:** 实验发现反直觉现象：在浅层或深层适度移除膜电位传播能提升性能，而过度移除会导致崩溃；在Something-Something V2、UCF101和HMDB51数据集上，STSep实现了优越性能，检索任务和注意力分析证实其专注于运动而非静态外观。

**Conclusion:** 研究揭示了时空资源竞争机制，即神经元在有限范围内同时编码语义和动态信息，时间状态消耗了空间学习容量；该工作为SNN的时序机制提供了新视角，并为视频理解中的时空建模提供了有效解决方案，推动了时空表示学习的发展。
...
### [4] [EventQueues: Autodifferentiable spike event queues for brain simulation on AI accelerators](https://arxiv.org/abs/2512.05906)
*Lennart P. L. Landsmeer, Amirreza Movahedin, Said Hamdioui, Christos Strydis*

#### 🧩 TL;DR
该研究提出了一种基于事件队列的脉冲神经网络梯度计算方法，实现了包含延迟的高效内存管理，并在多种硬件平台上进行了性能基准测试，揭示了不同队列设计对性能的关键影响。

---

#### 📘 Detailed Summary
**Motivation:** 脉冲神经网络的高效模拟和基于梯度的训练面临挑战，现有AI加速器虽能提供加速，但梯度计算通常使用内存密集的稠密数据结构实现稀疏脉冲事件，现有精确梯度方法缺乏通用性，且当前模拟器经常忽略或低效处理延迟脉冲。

**Method:** 研究通过脉冲事件队列推导梯度计算，包含延迟处理，并实现了内存高效、支持梯度的事件队列数据结构，在CPU、GPU、TPU和LPU平台上进行基准测试，探索了选择性脉冲丢弃作为性能-精度权衡的简单方法。

**Result:** 队列设计对性能有显著影响：CPU在传统树结构或FIFO实现中表现良好，GPU在较小模拟中环形缓冲区表现优异但在高内存压力下偏好稀疏数据结构，TPU倾向于基于排序内在函数的实现，选择性脉冲丢弃提供了有效的性能-精度权衡。

**Conclusion:** 研究揭示了不同硬件平台对脉冲神经网络事件队列数据结构的偏好差异，为未来自动微分框架设计提供了重要参考，建议框架应适应不同的原始/切向数据结构以实现更优的性能-精度权衡。
