...
#### 🧩 TL;DR
本文提出Signal-SGN++，一种拓扑感知的脉冲图框架，通过整合结构自适应性与时频脉冲动力学，在保持高能效的同时提升基于骨骼的动作识别性能，实现了优于现有SNN方法的精度-效率权衡。

---

#### 📘 Detailed Summary
**Motivation:** 图卷积网络在骨骼动作识别中表现出色，但其密集浮点计算能耗高；而脉冲神经网络虽具能效优势，却难以有效捕捉人体运动的耦合时频与拓扑依赖关系，因此需要一种能兼顾拓扑感知与能效的新框架。

**Method:** 提出Signal-SGN++框架，其主干网络由1D脉冲图卷积和频率脉冲卷积组成，用于联合时空与频谱特征提取；嵌入拓扑转移自注意力机制以自适应地在学习到的骨骼拓扑间路由注意力；并引入辅助的多尺度小波变换融合分支，通过拓扑感知时频融合单元整合结构先验以保持拓扑一致的频谱融合。

**Result:** 在大规模基准测试上的综合实验表明，Signal-SGN++实现了优越的精度-效率权衡，超越了现有基于SNN的方法，并在显著降低能耗的同时取得了与最先进GCNs相竞争的结果。

**Conclusion:** 该研究证明了将拓扑自适应机制与脉冲神经网络的时频动力学相结合的有效性，为高能效的骨骼动作识别提供了新范式，展示了在保持计算效率的同时提升图级敏感度的可行性，为未来节能型图神经网络设计指明了方向。
...
### [2] [LECalib: Line-Based Event Camera Calibration](https://arxiv.org/abs/2512.22441)
*Zibin Liu, Banglei Guana, Yang Shanga, Zhenbao Yu, Yifei Bian, Qifeng Yu*

#### 🧩 TL;DR
本文提出了一种基于线特征的事件相机标定框架，利用人造环境中常见物体的几何线条（如门窗、箱子等）进行标定，无需手动放置标定板，适用于单目和立体事件相机。

---

#### 📘 Detailed Summary
**Motivation:** 当前事件相机标定方法通常需要闪烁图案、重建强度图像或从事件中提取特征，这些方法通常耗时且需要手动放置标定物体，无法满足快速变化场景的需求。

**Method:** 该方法提出基于线的事件相机标定框架，直接从事件流中检测线条，利用事件-线标定模型生成相机参数的初始估计，适用于平面和非平面线条，然后采用非线性优化来细化相机参数。

**Result:** 仿真和真实世界实验验证了该方法的可行性和准确性，在单目和立体事件相机上进行了验证，源代码已在GitHub上开源。

**Conclusion:** 该方法通过利用环境中自然存在的几何线条，实现了无需专门标定物体的快速事件相机标定，为人造环境中的事件视觉应用提供了更实用的标定解决方案。
...
### [3] [Event-based high temporal resolution measurement of shock wave motion field](https://arxiv.org/abs/2512.22474)
*Taihang Lei, Banglei Guan, Minzu Liang, Pengju Sun, Jing Tao, Yang Shang, Qifeng Yu*

#### 🧩 TL;DR
该研究提出了一种基于多事件相机的新型框架，用于高时空分辨率测量冲击波运动参数，通过建立极坐标系编码事件、自适应ROI提取和迭代斜率分析等方法，实现了冲击波不对称性估计和运动场重建。

---

#### 📘 Detailed Summary
**Motivation:** 冲击波在功率场测试和损伤评估等应用中需要高时空分辨率的精确测量，但冲击波的快速不均匀传播和不稳定测试条件带来了重大挑战，现有方法难以满足这些要求。

**Method:** 该框架利用多事件相机的高速度和高动态范围能力，首先建立极坐标系编码事件以揭示冲击波传播模式，通过事件偏移计算实现自适应感兴趣区域提取；然后使用迭代斜率分析提取冲击波前沿事件，利用速度变化的连续性；最后根据基于事件的光学成像模型推导事件几何模型和冲击波运动参数，结合三维重建模型。

**Result:** 该方法实现了多角度冲击波测量、运动场重建和爆炸当量反演，速度测量结果与压力传感器和经验公式相比，最大误差为5.20%，最小误差为0.06%，实验结果表明该方法能够以高空间和时间分辨率实现冲击波运动场的高精度测量。

**Conclusion:** 该研究代表了冲击波测量技术的显著进步，通过事件相机框架实现了高时空分辨率的精确测量，为功率场测试和损伤评估等应用提供了有效的解决方案，展示了事件视觉在动态物理现象测量中的潜力。
...
### [4] [PoseStreamer: A Multi-modal Framework for 6DoF Pose Estimation of Unseen Moving Objects](https://arxiv.org/abs/2512.22979)
*Huiming Yang, Linglin Liao, Fei Ding, Sibo Wang, Zijian Zeng*

#### 🧩 TL;DR
本文提出了PoseStreamer，一个针对高速运动场景设计的鲁棒多模态6DoF姿态估计框架，通过集成自适应姿态记忆队列、物体中心2D跟踪器和射线姿态滤波器，显著提升了高速低光条件下的姿态估计性能。

---

#### 📘 Detailed Summary
**Motivation:** 在高速运动和低光照场景中，传统RGB相机因运动模糊导致6DoF姿态估计性能下降，而事件相机虽然具有高时间分辨率，但现有方法在高速物体运动场景中仍表现不佳，需要专门针对高速运动场景设计更鲁棒的姿态估计框架。

**Method:** 提出的PoseStreamer框架包含三个核心组件：自适应姿态记忆队列利用历史方向线索确保时间一致性，物体中心2D跟踪器提供强2D先验以提升3D中心召回率，以及射线姿态滤波器沿相机射线进行几何细化。此外，还构建了MoCapCube6D多模态数据集用于快速运动下的性能基准测试。

**Result:** 大量实验表明，PoseStreamer在高速运动场景中实现了卓越的精度，同时作为无模板框架对未见过的运动物体展现出强大的泛化能力。该框架在专门构建的MoCapCube6D数据集上进行了全面评估，验证了其在挑战性高速条件下的有效性。

**Conclusion:** 该研究展示了多模态融合和时间一致性机制在高速姿态估计中的重要性，为实时机器人操作和增强现实应用提供了实用解决方案。PoseStreamer的无模板特性使其能够泛化到未见物体，为动态环境中的鲁棒姿态估计开辟了新方向。
