...
### [1] [Detecting AI-Generated Videos with Spiking Neural Networks](https://arxiv.org/abs/2605.05895)
*Minsuk Jang, Yujin Yang, Heeseon Kim, Minseok Son, Younghun Kim, Changick Kim*

#### 🧩 TL;DR
本文提出MAST，一种基于脉冲神经网络（SNN）的AI生成视频检测器，利用视频帧间残差的多通道时空特征和语义特征轨迹，在跨生成器评估中达到93.14%的平均准确率，展示了SNN在该任务中的有效性。

---

#### 📘 Detailed Summary
**Motivation:** 现有AI生成视频检测方法在处理帧间时序证据时，要么采用通用时序骨干网络处理完整帧序列，要么将单一主导时序线索降维为固定视频级描述，要么通过与真实视频统计量比较的检测度量来对比时序特征；这些策略在跨生成器评估中性能急剧下降，因为不同生成器产生的伪影类型和时间尺度各不相同，且现有检测器未能联合利用像素级时序残差平滑性与语义特征空间轨迹紧凑性这两个AI视频的共有特征。

**Method:** MAST（Multi-channel Artifact Spatio-Temporal detector）包含两分支架构：一个基于SNN的脉冲驱动时序分支，处理多通道帧间时序残差，利用SNN的事件驱动、稀疏激活特性响应集中在物体和运动边缘的时序伪影；另一个为冻结的语义编码器分支，用于提取语义特征轨迹；两分支联合实现跨生成器的泛化检测。

**Result:** 在GenVideo基准上，MAST在严格的跨生成器评估下，对10个未见生成器达到93.14%的平均准确率，匹配甚至超越最强的基于ANN的检测器，验证了SNN在AI生成视频检测中的实际应用可行性。

**Conclusion:** AI生成视频在像素级时序残差和语义特征空间轨迹上均表现出比真实视频更平滑的时间动态，形成可检测的时序平滑度差距；SNN的脉冲激活集中在物体和运动边界处，其事件驱动、稀疏激活的动态特性与残差信号的结构天然契合，为跨生成器泛化提供了新的检测范式。
...
### [2] [Neuromorphic visual attention for Sign-language recognition on SpiNNaker](https://arxiv.org/abs/2605.06005)
*Sarka Liskova, Olha Vedmedenko, Mazdak Fatahi, Matej Hoffmann, P. Michael Furlong, Giulia D Angelo*

#### 🧩 TL;DR
本文提出了一种端到端的神经形态架构，集成了脉冲视觉注意力机制与紧凑型脉冲神经网络，用于美国手语手指拼写识别，在SpiNNaker平台上实现，在模拟和硬件部署上分别达到92.27%和83.1%的准确率，同时实现了0.565 mW的极低功耗和3 ms的低延迟。

---

#### 📘 Detailed Summary
**Motivation:** 现有手语识别方法虽在分类准确率上取得了显著进展，但其高延迟和高功耗限制了在实时场景中的部署可行性；神经形态感知与处理基于稀疏事件驱动计算，为低延迟、低功耗感知提供了新范式，但仍缺乏针对手语识别的完整端到端神经形态解决方案。

**Method:** 本文设计了一种端到端神经形态架构，包含一个脉冲视觉注意力机制用于在线感兴趣区域提取，以及一个紧凑型脉冲神经网络（SNN）用于分类；整个系统部署在SpiNNaker神经形态硬件平台上，并在合成事件版Sign Language MNIST数据集和原生记录的ASL-DVS数据集上进行评估。

**Result:** 所提系统在仿真中取得92.27%的分类准确率，在神经形态硬件部署上达到83.1%的准确率；功耗仅为0.565 mW，延迟为3 ms，在所有对比方法中实现了最优的能量效率和最低延迟，展现出紧凑架构下的竞争力。

**Conclusion:** 该工作证明了将任务相关的视觉注意力机制与脉冲神经网络相结合在边缘端部署手语识别任务的可行性，尽管模型紧凑，仍能在低功耗和低延迟约束下取得有竞争力的性能，为实时神经形态感知应用提供了有效方案。
...
### [3] [Dynamic Pondering Sparsity-aware Mixture-of-Experts Transformer for Event Stream based Visual Object Tracking](https://arxiv.org/abs/2605.06112)
*Shiao Wang, Xiao Wang, Duoqing Yang, Wenhao Zhang, Bo Jiang, Lin Zhu, Yonghong Tian, Bin Luo*

#### 🧩 TL;DR
本文提出一种事件稀疏性感知跟踪框架，通过多尺度事件密度注入、稀疏感知混合专家模块和动态推理深度调整策略，在事件相机跟踪任务中实现了精度与效率的良好平衡。

---

#### 📘 Detailed Summary
**Motivation:** 现有RGB跟踪器在低光照和快速运动等挑战性成像条件下表现脆弱，而现有基于事件相机的跟踪器忽视了事件数据固有的空间稀疏性和时间密集性，且依赖单一固定时间窗口采样策略，难以适应变化的运动动态。

**Method:** 提出一种事件稀疏性感知跟踪框架，将稀疏、中等密度和密集的事件搜索区域逐步注入三级Vision Transformer骨干网络，实现层次化多密度特征学习；引入稀疏感知混合专家模块以鼓励专家在不同稀疏模式下的专业化，并设计动态推理策略根据跟踪难度自适应调整推理深度。

**Result:** 在FE240hz、COESOT和EventVOT三个基准数据集上的大量实验表明，该方法在跟踪精度与计算效率之间取得了良好的权衡。

**Conclusion:** 该研究揭示了显式建模事件数据多尺度密度变化对于提升事件相机跟踪性能的重要性，所提出的动态推理机制为实现精度-效率折衷提供了有效途径，未来可通过开源框架进一步推动该领域发展。
