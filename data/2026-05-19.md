...
#### 🧩 TL;DR
提出了一种基于光子统计的事件相机（EC）基础概率模型，统一了静态场景噪声与S曲线的描述，并基于该模型开发了Noise2Params方法，仅需静态均匀场景即可标定相机参数，合成噪声数据训练的CNN在实验数据上的表现优于纯实验数据训练的模型。

---

#### 📘 Detailed Summary
**Motivation:** 事件相机（EC）缺乏精确且统一的模型，阻碍了校准和算法设计的进展；现有的方法要么依赖于专门的动态光源，要么无法统一描述静态场景噪声事件和阶跃响应曲线（S-curves）这两种看似无关的行为。

**Method:** 基于光子统计推导了事件检测的概率模型，覆盖全部强度区间的三种分布形式（精确Poisson、鞍点近似、Gaussian）；提出Noise2Params方法，通过最小化观测噪声事件分布与模型之间的误差来标定相机参数（对数对比度阈值B、勒克斯-光子转换因子α、泄漏项θ）；利用合成的噪声图像训练CNN以重建静态场景。

**Result:** Noise2Params仅需静态均匀场景即可标定相机参数，且发现泄漏项θ具有强度依赖性；合成噪声数据训练的CNN在实验数据上表现优于仅用实验数据训练的CNN，验证了模型的有效性。

**Conclusion:** 该框架为事件相机的定量校准、噪声感知算法设计以及在光子受限条件下的应用提供了理论基础，揭示了S曲线解释的微妙性（并非简单的固定概率阈值选择），并提供了实验上易于实现的参数标定途径。
...
### [2] [NERVE: A Neuromorphic Vision and Radar Ensemble for Multi-Sensor Fusion Research](https://arxiv.org/abs/2605.16414)
*Omar Mansour, Pietro Martinello, Ethan Milon, YingFu Xu, Manolis Sifalakis, Guangzhi Tang, Amirreza Yousefzadeh*

#### 🧩 TL;DR
NERVE是一个包含257分钟多传感器同步记录（DVS、RGB-D相机、24GHz和77GHz雷达）的大规模数据集，通过基线实验证明DVS与77GHz雷达融合可显著提升人检测与距离估计性能，循环检测器达到47.5% mAP且距离误差低于1.8m。

---

#### 📘 Detailed Summary
**Motivation:** 现有神经形态视觉与雷达融合研究缺乏大规模、多传感器同步标注的真实场景数据集，难以系统评估多模态融合在动态感知任务中的协同效果。

**Method:** 论文构建了NERVE数据集，包含两个DVS、一个RGB-D相机和两个雷达单元（24GHz和77GHz）的同步记录，共计约600GB数据、914,000帧和960万RGB COCO格式标注，涵盖16类目标；并构建DVS+Radar子集用于人检测与距离估计，使用前馈和循环检测器作为基线模型评估多模态融合性能。

**Result:** 基线实验表明，DVS与77GHz雷达融合相比单模态一致提升检测性能，循环检测器达到47.5% mAP，且平均雷达距离估计误差相对于LiDAR真值低于1.8m。

**Conclusion:** NERVE数据集为神经形态视觉与雷达融合研究提供了高质量多模态基准，验证了DVS与77GHz雷达融合在检测与距离估计任务上的有效性，为该方向后续研究奠定了数据与方法基础。
...
### [3] [NeuroLiDAR: Adaptive Frame Rate Depth Sensing via Neuromorphic Event-LiDAR Fusion](https://arxiv.org/abs/2605.16805)
*Darshana Rathnayake, Dulanga Weerakoon, Meera Radhakrishnan, Archan Misra*

#### 🧩 TL;DR
NeuroLiDAR 提出一种自适应深度感知框架，通过融合稀疏 LiDAR 数据与密集事件相机输入，实现高达约66 Hz的有效帧率，并在深度重建误差上降低约29%。

---

#### 📘 Detailed Summary
**Motivation:** LiDAR 系统受限于硬件约束，在探测范围、空间分辨率和帧率之间存在固有折衷，通常以低帧率（5-10 Hz）运行以优先保障远距离感知，难以适应快速变化的场景动态。

**Method:** NeuroLiDAR 融合时间稀疏的 LiDAR 数据与时间密集的神经形态事件相机输入，通过事件驱动的关键帧检测和事件引导的深度外推两个核心组件，根据场景动态自适应调整感知帧率。

**Result:** 在自建 ELiDAR 数据集（涵盖室内外场景）上，NeuroLiDAR 在深度重建的均方根误差（RMSE）上降低了约29%，同时实现了27.8–47.3 Hz的自适应帧率。

**Conclusion:** NeuroLiDAR 证明通过事件相机与 LiDAR 的异源融合，能够在不牺牲感知范围的前提下显著提升有效帧率和深度重建精度，为突破 LiDAR 硬件限制提供了有效途径。
...
### [4] [Brain-inspired spike-timing plasticity for reliable label-efficient event-camera vision](https://arxiv.org/abs/2605.17686)
*Mohamad Yazan Sadoun, Sarah Sharif, Yaser Mike Banad*

#### 🧩 TL;DR
本文提出基于局部STDP的三模块无GPU框架，在事件相机目标检测中实现零标签监督（53.8% mAP@30）和极低标签高效检测，并证明其在不依赖梯度训练与密集矩阵乘法条件下超越传统方法。

---

#### 📘 Detailed Summary
**Motivation:** 事件相机目标检测面临两个核心瓶颈：逐帧标注成本高昂以及推理对GPU算力的强依赖。现有方法无法同时满足低标签需求、轻量计算（单CPU）与无梯度/无密集矩阵乘法约束，亟需一种不依赖梯度训练和密集矩阵乘法的局部可塑性方案。

**Method:** 提出三个局部STDP模块：序列模块、候选模块和管道可靠性模块，全部运行在单CPU线程上无需GPU。构建三层次标签高效监督框架：严格零标签检测器、约26位训练衍生比特的检测器、以及STDP候选可靠性门控模块。门控模块还包括一个基于队列的可靠性门控机制用于抗偏移场景，并在管道层面引入STDP层进行虚警抑制。

**Result:** 在FRED无人机基准上：零标签检测器达53.8% mAP@30；约26个训练衍生比特达76.9% mAP@30；STDP候选可靠性门控达78.60±0.42% mAP@30。在采集顺序漂移下，队列门控在20/20次正试验中平均超越流式k-means 2.03±0.58个百分点，无漂移对照验证了效果显著性。STDP使单模型方差降低6.6倍，单个训练好的门控等效于44个种子的集成上界。该门控迁移至Intel Lava框架达89% top-2一致性。在EVUAV基准上，管道级STDP层在Pd≥88%条件下将虚警从454降至331e-4。

**Conclusion:** 本文证明局部STDP模块可在无梯度训练、无GPU、无密集矩阵乘法条件下实现高水平事件相机目标检测，且具备低标签依赖、低方差、抗分布漂移等优势，与Lava框架的兼容性表明其可嵌入神经形态硬件生态，为事件视觉提供了一种全新且资源高效的范式。
...
### [5] [Spiker-LL: An Energy-Efficient FPGA Accelerator Enabling Adaptive Local Learning in Spiking Neural Networks](https://arxiv.org/abs/2605.18003)
*Alessio Caviglia, Filippo Marostica, Alessandro Savino, Stefano Di Carlo*

#### 🧩 TL;DR
本文提出SPIKER-LL，一种基于FPGA的脉冲神经网络加速器，通过微架构扩展支持STSF局部学习规则，在边缘设备上实现高效的在线学习，在多个数据集上达到93%准确率、亚毫秒延迟和低于0.1mJ每推理的能耗，且无需DSP单元。

---

#### 📘 Detailed Summary
**Motivation:** 在边缘设备上部署自适应智能面临神经模型训练带来的高计算和能耗成本的挑战，脉冲神经网络虽提供有潜力的替代方案，但实现设备端学习需要硬件与算法的协同设计，现有方案缺乏高效支持在线学习的轻量化硬件加速器。

**Method:** SPIKER-LL在开源Spiker+推理架构基础上，通过针对性的微架构扩展，引入对STSF（Spatio-Temporal Spike Feature）局部学习规则的高效硬件支持，使其能够在FPGA上同时执行推理和在线学习，且不依赖DSP单元，保持高度可扩展性。

**Result:** 在MNIST、F-MNIST和DIGITS数据集上，SPIKER-LL达到最高93%的准确率，推理延迟低于亚毫秒级别，每次推理能耗小于0.1mJ，验证了其在边缘FPGA部署中的高效性与实用性。

**Conclusion:** SPIKER-LL证明了通过硬件-算法协同设计，可以在资源受限的边缘设备上实现高效的SNN在线学习，其无DSP、低功耗、低延迟和可扩展的特性为边缘智能部署提供了可行的解决方案和设计参考。
