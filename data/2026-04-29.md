...
### [1] [Vision SmolMamba: Spike-Guided Token Pruning for Energy-Efficient Spiking State-Space Vision Models](https://arxiv.org/abs/2604.25570)
*Dewei Bai, Hongxiang Peng, Yunyun Zeng, Ziyu Zhang, Hong Qu, Yi Zhang*

#### 🧩 TL;DR
本文提出 Vision SmolMamba，一种将脉冲驱动动力学与线性时间选择性状态空间递归相结合的能量高效脉冲视觉架构，通过脉冲引导的空时令牌剪枝器（SST-TP）实现冗余令牌的渐进式移除，在多个静态和事件驱动基准上以至少1.5倍的能耗降低取得了优于或持平于脉冲Transformer和Spiking Mamba基线的精度-效率权衡。

---

#### 📘 Detailed Summary
**Motivation:** 现有的Spiking Transformers虽通过脉冲驱动自注意力实现了长程视觉建模，但其二次复杂度的令牌交互从根本上与脉冲神经计算的稀疏、事件驱动特性不匹配，导致计算效率低下。

**Method:** 提出Vision SmolMamba架构，核心包括一个脉冲引导的空时令牌剪枝器（SST-TP），利用脉冲激活强度和首次脉冲延迟估计令牌重要性，逐步移除冗余令牌并保留关键空时信息；在此基础上，SmolMamba模块将脉冲事件直接集成到双向状态空间递归中，构成高效的脉冲状态空间视觉骨干网络。

**Result:** 在ImageNet-1K、CIFAR10/100、CIFAR10-DVS和DVS128 Gesture等多个静态与事件驱动基准上，Vision SmolMamba在保持竞争性甚至更高精度的同时，相比之前的脉冲Transformer基线和Spiking Mamba变体，估计能耗至少降低了1.5倍。

**Conclusion:** 将脉冲引导的令牌稀疏性与状态空间建模相结合，为脉冲视觉系统提供了一种可扩展且能量高效的范式，表明通过利用脉冲本身的稀疏特性来驱动递归动态是一种有前景的方向。
...
### [2] [QB-LIF: Learnable-Scale Quantized Burst Neurons for Efficient SNNs](https://arxiv.org/abs/2604.25688)
*Dewei Bai, Hongxiang Peng, Jiajun Mei, Yang Ren, Hong Qu, Dawen Xia, Zhang Yi*

#### 🧩 TL;DR
提出Quantized Burst-LIF (QB-LIF)神经元，将爆发脉冲重定义为带可学习尺度的饱和均匀量化，通过可吸收尺度策略和ReLSG-ET代理梯度实现高效多比特脉冲通信与训练，在多种基准上超越二值和固定爆发SNN。

---

#### 📘 Detailed Summary
**Motivation:** 传统脉冲神经网络（SNN）中二进制脉冲编码每时间步仅传递1比特信息，在深层架构和短仿真步数下信息吞吐量严重受限，成为制约精度和效率的关键瓶颈。

**Method:** 提出Quantized Burst-LIF (QB-LIF)神经元，将爆发脉冲建模为膜电位的饱和均匀量化，量化尺度作为可学习参数由各层自适应调整；引入可吸收尺度策略，在推理时将量化尺度融合入突触权重，保持纯累加（AC）执行范式；设计ReLSG-ET代理梯度，利用指数尾部修正线性函数以维持爆发间隔内的梯度流动，支持离散多级空间上的稳定优化。

**Result:** 在CIFAR-10/100、ImageNet等静态数据集和CIFAR10-DVS、DVS128-Gesture等事件驱动基准上，QB-LIF在超低延迟条件下显著优于二值SNN和固定爆发SNN，取得了更高精度并保持了神经形态兼容性。

**Conclusion:** 通过将爆发脉冲重新解释为可学习尺度的量化过程，QB-LIF在提升脉冲信息吞吐量的同时保留了SNN的事件驱动和硬件高效特性，为深层SNN在超低延迟场景下的实用化提供了有效方案。
