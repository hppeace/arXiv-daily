...
### [1] [A neuromorphic model of the insect visual system for natural image processing](https://arxiv.org/abs/2602.06405)
*Adam D. Hines, Karin Nordström, Andrew B. Barron*

#### 🧩 TL;DR
本文提出了一种受昆虫视觉启发的生物启发视觉模型，通过完全自监督的对比学习目标生成稀疏、可区分的视觉编码，该模型在花朵识别任务和自然图像基准测试中表现出色，并实现了人工神经网络和脉冲神经网络两种实现形式。

---

#### 📘 Detailed Summary
**Motivation:** 当前许多计算模型过于关注任务性能而忽视了生物真实的处理路径，本研究旨在填补这一空白，通过捕捉昆虫视觉系统的原理来构建更符合生物机制的视觉处理模型。

**Method:** 该研究引入了一个生物启发视觉模型，将密集视觉输入转换为稀疏、可区分的编码，采用完全自监督的对比学习目标进行训练，无需标注数据即可学习表示，并支持跨任务重用而无需依赖特定领域分类器，同时实现了人工神经网络和脉冲神经网络两种实现形式。

**Result:** 模型在花朵识别任务和自然图像基准测试中持续产生可靠的稀疏编码，能够区分视觉相似的输入，在模拟定位设置中，该方法优于简单的图像下采样基线，突显了整合神经形态视觉处理路径的功能优势。

**Conclusion:** 这些结果推进了昆虫计算建模领域，提供了一个通用的生物启发视觉模型，能够在多样化任务中执行稀疏计算，为理解生物视觉处理提供了新的计算框架，并展示了自监督学习在生物启发模型中的有效性。
...
### [2] [Sparse Spike Encoding of Channel Responses for Energy Efficient Human Activity Recognition](https://arxiv.org/abs/2602.06766)
*Eleonora Cicciarella, Riccardo Mazzieri, Jacopo Pegoraro, Michele Rossi*

#### 🧩 TL;DR
本文提出了一种用于集成传感与通信（ISAC）中人类活动识别的脉冲卷积自编码器（SCAE），该模型能够学习信道冲激响应（CIR）的稀疏脉冲编码表示，并与脉冲神经网络（SNN）联合训练，在保持高准确率的同时显著降低能耗。

---

#### 📘 Detailed Summary
**Motivation:** 集成传感与通信（ISAC）系统需要边缘设备进行持续监测，但现代传感算法通常过于复杂，无法满足能量受限边缘设备的能效要求。脉冲神经网络（SNNs）因其处理稀疏二进制脉冲序列的特性，有望将能耗降低数个数量级，但需要开发能够有效学习传感数据表示的脉冲编码方法。

**Method:** 本文提出了一种脉冲卷积自编码器（SCAE），专门学习信道冲激响应（CIR）数据的定制化脉冲编码表示。该模型与用于人类活动识别（HAR）的脉冲神经网络（SNN）进行联合训练，从而消除了传统方法中对多普勒域预处理的需求，实现了从原始CIR数据到分类结果的端到端脉冲处理。

**Result:** 实验结果表明，SCAE-SNN模型在人类活动识别任务中达到了与混合方法相当的F1分数（接近96%），同时生成了显著更稀疏的脉冲编码（81.1%的稀疏度）。在分类前对CIR数据进行编码不仅提高了HAR的准确性，还显著提升了系统的能效。

**Conclusion:** 该研究证明了脉冲编码自编码器在ISAC系统中处理传感数据的有效性，为能量受限边缘设备上的实时感知任务提供了一种高效解决方案。联合训练方法消除了预处理需求，简化了系统架构，而高稀疏度的脉冲编码为实现超低功耗的传感算法奠定了基础。
