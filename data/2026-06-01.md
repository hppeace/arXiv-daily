...
### [1] [Gradient-Free Training of Spiking Neural Networks via Low-Rank Evolution Strategies](https://arxiv.org/abs/2605.30361)
*Dhruv Patankar, Sachit Ramesha Gowda*

#### 🧩 TL;DR
提出EGGROLL方法，通过低秩分解ES扰动将每代记忆复杂度从O(mn)降至O(r(m+n))，实现了无需梯度、兼容神经形态硬件的SNN训练，在N-MNIST上达到79.21%准确率并加速2.23倍。

---

#### 📘 Detailed Summary
**Motivation:** SNN的离散脉冲阈值不可微，替代梯度方法虽能近似导数但需反向传播基础设施，无法兼容片上学习；进化策略虽无需梯度但计算开销随参数数量线性增长，难以应用于大规模权重矩阵，因此亟需一种高效且与神经形态硬件兼容的无梯度SNN训练方法。

**Method:** 提出EGGROLL方法，采用低秩分解对进化策略的扰动矩阵进行因式分解，将每代记忆复杂度从O(mn)降至O(r(m+n))；结合Leaky Integrate-and-Fire（LIF）SNN模型在N-MNIST数据集上进行无梯度训练。

**Result:** 在N-MNIST数据集上，EGGROLL方法达到79.21%的测试准确率，同时每代运行时间较全秩进化策略减少2.23倍，表明该方法在SNN训练中可行且具有明确的准确率-速度权衡。

**Conclusion:** EGGROLL证明了无替代梯度的进化策略在SNN训练中的可行性，其低秩分解策略能有效降低计算开销，同时保持与神经形态硬件的兼容性，为SNN的片上学习提供了一条新途径。
...
### [2] [XOResNet: Exclusive-OR Meta-Residuals Facilitate Deep Spiking Neural Networks Learning](https://arxiv.org/abs/2605.30362)
*Jianfang Wu, Junsong Wang*

#### 🧩 TL;DR
提出OR-ADD (OA)短路连接和XOR元残差机制，构建XOR残差块与XOResNet，解决SNN残差结构中的脉冲冗余、信息丢失和冗余学习问题，在多个数据集上超越现有SNN方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有深度脉冲神经网络（SNN）的残差结构存在恒等映射中的脉冲冗余、非恒等映射中的信息丢失以及骨干分支中的冗余学习等挑战，限制了深层SNN的训练效率和表示能力。

**Method:** 提出OR-ADD (OA)短路连接，用于合并残差结构中两个分支的输出脉冲/电流以缓解信号冗余与信息丢失；引入XOR元残差概念，通过异或运算为骨干分支选择预学习残差以减少冗余学习；将OA短路与XOR元残差集成构建XOR残差块，并基于该块构建不同深度的XOResNet。

**Result:** 在Fashion-MNIST、CIFAR-10、CIFAR-100和miniImageNet四个数据集上的实验表明，XOResNet在分类性能上优于现有基于梯度下降优化的深度SNN方法，验证了OA短路连接和XOR元残差模块在解决SNN残差学习中的有效性。

**Conclusion:** 本工作通过OA短路和XOR元残差机制克服了SNN中残差学习的基本局限性，为构建高性能神经形态系统提供了新的架构设计思路和指导。
...
### [3] [Reinterpreting Safety Thresholds as Neuron Spiking Thresholds](https://arxiv.org/abs/2605.30368)
*Enrico Del Re, Mohamed Sabry, Cristina Olaverri-Monreal*

#### 🧩 TL;DR
本文提出将替代安全度量（SSM）的固定阈值重新解释为脉冲神经网络（SNN）中LIF神经元的激发阈值，通过学习与人类刹车行为对齐的脉冲活动，实现客观SSM与主观安全感知的融合。

---

#### 📘 Detailed Summary
**Motivation:** 现有SSM评估方法大多采用固定阈值，无法捕捉人类在持续临界状态下的响应行为，也难以反映对短暂高风险峰值的反应，导致客观安全度量与主观人类安全感知之间存在鸿沟。

**Method:** 本文提出一种受生物启发的SSM阈值重新解释方法，将多个SSM输入组合到基于泄漏整合-发放（LIF）神经元的脉冲神经网络（SNN）中，SNN被训练用于发射与人类刹车开始时刻对齐的脉冲；训练数据来自使用3D-CoAutoSim平台（基于CARLA/Unreal和六自由度运动平台）的受控跟车实验，其中诱导产生了关键临界事件。

**Result:** 实验结果表明，学习到的脉冲活动在多个场景下与刹车行为定性对齐，并能捕获那些不能仅由阈值跨过一致解释的反应；跨参与者分析显示，学习到的输入阈值相对一致，而学习到的衰减因子编码了不同SSM的时间敏感度差异。

**Conclusion:** 本研究表明脉冲动力学可以作为促使客观SSM与主观人类安全感知趋于一致的机制，为自动驾驶风险评估提供了融合生物启发计算的新方向。
...
### [4] [Memristor-Based Spiking Neural Network Accelerator for Bio-inspired Interception Task](https://arxiv.org/abs/2605.31299)
*Qianhou Qu, Sheng Lu, Liuting Shang, Jaihan Utailawon, Sungyong Jung, Qilian Liang, Chenyun Pan*

#### 🧩 TL;DR
本文提出了一种基于模拟忆阻器的脉冲神经网络加速器，通过存内突触计算与模拟积分-放电神经元实现异步事件驱动运算，相比5nm数字基线的能耗降低12.7倍，延迟降低1.26倍，在捕食者-猎物追踪任务中推理MSE仅为0.004，展示了忆阻器神经形态电路在高效边缘智能中的应用潜力。

---

#### 📘 Detailed Summary
**Motivation:** 现有脉冲神经网络（SNN）实际部署仍依赖基于冯·诺依曼架构的GPU和CPU平台，存储与计算分离导致严重的能效瓶颈，亟需探索非冯·诺依曼的硬件加速方案以释放SNN事件驱动、低功耗的潜在优势。

**Method:** 提出一种基于模拟忆阻器的SNN加速器架构，将存内突触计算与模拟积分-放电（IF）神经元集成，消除了传统多晶体管CMOS突触电路，实现异步事件驱动运算，设计基于45nm工艺节点；同时设计并优化了5nm工艺节点的数字SNN加速器作为对比基线，采用捕食者-猎物追踪任务作为评估基准。

**Result:** 在捕食者-猎物追踪任务中，模拟SNN加速器的推理结果与理想软件推理高度吻合，均方误差（MSE）仅为0.004；HSPICE仿真结果表明，相比5nm数字基线，所提模拟SNN加速器能耗降低12.7倍，延迟降低1.26倍。

**Conclusion:** 忆阻器基神经形态电路能够显著超越先进工艺节点（5nm）数字方案的能量效率和延迟表现，验证了通过存内计算与模拟神经元实现高效实时边缘智能的可行性和巨大潜力，为SNN硬件加速提供了有前景的非冯·诺依曼实现路径。
