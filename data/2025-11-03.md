...
### [1] [Spiking Neural Networks: The Future of Brain-Inspired Computing](https://arxiv.org/abs/2510.27379)
*Sales G. Aribe Jr*

#### 🧩 TL;DR
本研究对脉冲神经网络进行了全面分析，表明SNN在保持接近ANN精度的同时，能够实现显著的能量效率和低延迟特性，特别适合能量受限的边缘AI应用。

---

#### 📘 Detailed Summary
**Motivation:** 脉冲神经网络作为新一代神经计算模型，旨在解决传统人工神经网络在能量效率和时序动态特性方面的局限性，为能量受限、延迟敏感的应用场景提供更优的解决方案。

**Method:** 研究深入分析了LIF等关键神经元模型，并评估了多种训练策略，包括替代梯度下降法、ANN到SNN转换方法以及基于脉冲时序依赖可塑性的无监督学习算法。

**Result:** 实验结果显示替代梯度训练的SNN在精度上与ANN相差仅1-2%，收敛速度更快，延迟低至10毫秒；转换的SNN性能竞争性但需要更多脉冲；STDP训练的SNN虽然收敛较慢，但具有最低的脉冲计数和能量消耗，每推理仅需5毫焦耳。

**Conclusion:** 研究证实SNN特别适合机器人技术、神经形态视觉和边缘AI系统等应用场景，尽管在硬件标准化和可扩展训练方面仍存在挑战，但经过进一步优化后有望推动神经形态计算的下一个发展阶段。

---

#### 📄 Abstract
Spiking Neural Networks (SNNs) represent the latest generation of neural
...
### [2] [Exploiting heterogeneous delays for efficient computation in low-bit neural networks](https://arxiv.org/abs/2510.27434)
*Pengfei Sun, Jascha Achterberg, Zhe Su, Dan F. M. Goodman, Danyal Akarca*

#### 🧩 TL;DR
该研究提出通过共同学习突触权重和延迟参数，利用延迟异质性实现脉冲神经网络在时间复杂任务上的最先进性能，即使在权重精度极低（1.58位三元精度）的情况下也能保持高性能，为神经形态计算提供了高效的内存解决方案。

---

#### 📘 Detailed Summary
**Motivation:** 传统神经网络主要关注突触权重学习，但忽略了其他可学习的神经参数如延迟，而大脑表现出复杂的异质性延迟时间动态，其中信号在神经元间异步传输，这种延迟异质性可能在具身环境中被利用来处理时间域中的任务相关信息。

**Method:** 研究训练脉冲神经网络同时修改权重和不同精度水平的延迟参数，探索延迟和权重在多种精度配置下的协同学习机制，分析延迟异质性对网络性能的影响。

**Result:** 延迟异质性使得脉冲神经网络在时间复杂神经形态问题上达到最先进性能，即使在权重精度极低（1.58位三元精度）时也能保持高性能，实现比典型仅权重网络更激进的压缩（超过一个数量级），且时间复杂任务需要更长的延迟分布。

**Conclusion:** 时间异质性是高效计算的重要原则，特别是在任务相关信息具有时间特性的物理世界中，延迟异质性允许在保持最先进精度的同时实现内存高效解决方案，对具身智能系统和神经形态硬件具有重要启示意义。

---

#### 📄 Abstract
Neural networks rely on learning synaptic weights. However, this overlooks
