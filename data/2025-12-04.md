...
### [1] [Hybrid Temporal-8-Bit Spike Coding for Spiking Neural Network Surrogate Training](https://arxiv.org/abs/2512.03879)
*Luu Trong Nhan, Luu Trung Duong, Pham Ngoc Nam, Truong Cong Thang*

#### 🧩 TL;DR
该研究提出了一种混合时空比特脉冲编码方法，将比特平面分解与时间编码原理相结合，首次为SNN的代理梯度训练设计了专门的混合时空比特编码方案，在多个视觉基准测试中取得了具有竞争力的性能。

---

#### 📘 Detailed Summary
**Motivation:** 脉冲神经网络在视觉任务中难以达到最先进性能，尽管混合速率-时间编码策略（特别是结合比特平面表示的方法）已显示出潜力，但现有方法在代理梯度训练方面仍有改进空间，本研究旨在解决这一性能差距。

**Method:** 本研究提出了一种混合时空比特脉冲编码方法，该方法将比特平面分解与时间编码原理相结合，专门为代理梯度训练的SNN设计，通过整合比特平面信息和时间编码机制来增强信息表示能力。

**Result:** 在多个计算机视觉基准测试上的广泛实验表明，该方法相比已建立的脉冲编码技术取得了具有竞争力的性能，在某些情况下甚至实现了性能改进，验证了混合时空比特编码的有效性。

**Conclusion:** 这是首个专门为SNN代理梯度训练设计的混合时空比特编码方案，研究结果表明结合比特平面分解与时间编码原理能够有效提升SNN在视觉任务中的性能，为未来脉冲编码策略的发展提供了新方向。
...
### [2] [Parameter efficient hybrid spiking-quantum convolutional neural network with surrogate gradient and quantum data-reupload](https://arxiv.org/abs/2512.03895)
*Luu Trong Nhan, Luu Trung Duong, Pham Ngoc Nam, Truong Cong Thang*

#### 🧩 TL;DR
本研究提出了一种新颖的Spiking-Quantum Data Re-upload卷积神经网络（SQDR-CNN）架构，能够在单一反向传播框架中联合训练卷积脉冲神经网络和量子电路，无需依赖预训练的脉冲编码器，显著减少了模型参数需求。

---

#### 📘 Detailed Summary
**Motivation:** 现有Spiking Quantum Neural Network（SQNN）实现通常依赖于预训练的脉冲神经网络，这是由于脉冲活动的不可微分性和当前SNN编码器的有限可扩展性所导致的限制，这阻碍了脉冲神经网络与量子电路的联合优化训练。

**Method:** 本研究提出了Spiking-Quantum Data Re-upload卷积神经网络（SQDR-CNN）架构，该架构将卷积脉冲神经网络与量子电路集成在单一可微框架中，采用量子数据重上传技术，并在噪声模拟量子环境下测试了不同训练算法和初始化策略的设计。

**Result:** SQDR-CNN在无需预训练脉冲编码器和数据集子集的情况下实现了合理性能收敛，达到了最先进SNN基线模型平均最高准确率的86%，同时仅使用最小脉冲模型参数的0.5%，显著减少了参数需求。

**Conclusion:** 该研究通过整合神经形态计算和量子计算范式，为多模态可学习系统开辟了新的研究方向，展示了在单一框架中联合优化脉冲和量子组件的可行性，为未来混合计算架构的发展提供了理论基础和技术路径。
