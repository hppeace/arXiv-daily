...
### [1] [Personalized Spiking Neural Networks with Ferroelectric Synapses for EEG Signal Processing](https://arxiv.org/abs/2601.00020)
*Nikhil Garg, Anxiong Song, Niklas Plessnig, Nathan Savoia, Laura Bégon-Lours*

#### 🧩 TL;DR
该研究展示了尖峰神经网络在铁电忆阻突触器件上实现自适应脑电运动想象解码的可行性，提出了一种设备感知的权重更新策略，在真实器件约束下实现了与软件SNN相当的分类性能。

---

#### 📘 Detailed Summary
**Motivation:** 基于脑电图的脑机接口受到非平稳神经信号的影响，这些信号在不同会话和个体间存在差异，限制了主体无关模型的泛化能力，需要在资源受限平台上实现自适应和个性化学习，而可编程忆阻硬件为此提供了有前景的基底，但实际实现受到权重分辨率有限、器件变异性、非线性编程动态和有限器件耐久性等挑战。

**Method:** 研究制造、表征并建模了铁电突触器件，评估了卷积-循环尖峰神经网络架构在两种互补部署策略下的表现：使用铁电突触模型进行设备感知训练，以及软件训练权重转移后进行低开销片上重调优，并引入了一种设备感知权重更新策略，其中基于梯度的更新在数字上累积，仅在超过阈值时转换为离散编程事件，以模拟非线性状态依赖编程动态同时减少编程频率。

**Result:** 两种部署策略均实现了与最先进软件尖峰神经网络相当的分类性能，通过仅重训练最终网络层实现的主体特定迁移学习进一步提高了分类准确率，证明了在真实器件约束下铁电硬件能够支持稳健的低开销自适应。

**Conclusion:** 该研究表明可编程铁电硬件能够支持尖峰神经网络中的稳健低开销自适应，为神经信号的个性化神经形态处理开辟了实用路径，设备感知训练和权重转移策略为在资源受限平台上实现自适应脑机接口提供了可行方案。
...
### [2] [Modern Neuromorphic AI: From Intra-Token to Inter-Token Processing](https://arxiv.org/abs/2601.00245)
*Osvaldo Simeone*

#### 🧩 TL;DR
本文系统阐述了现代神经形态AI模型通过"令牌内处理"和"令牌间处理"的视角，揭示了神经形态计算原理、状态空间模型与Transformer架构之间的内在联系，并综述了相应的训练方法学。

---

#### 📘 Detailed Summary
**Motivation:** 人工智能的快速发展带来了强大的数据处理和生成能力，但同时也伴随着急剧增长的能耗需求，这促使研究者重新关注神经形态计算原理，该原理通过离散稀疏激活、循环动力学和非线性反馈机制，有望实现类似大脑的高能效计算。

**Method:** 论文通过"令牌内处理"和"令牌间处理"的区分框架系统分析现代神经形态AI模型，其中令牌内处理主要基于脉冲神经网络对同一向量输入的多通道特征进行变换，而令牌间处理则利用状态空间动力学或稀疏自注意力机制，根据上下文相关性选择性地组合不同信息元素，实现关联记忆机制。

**Result:** 研究建立了神经形态模型、状态空间模型和Transformer架构之间的系统性联系，展示了现代AI架构如何通过重度量化激活、状态空间动力学和稀疏注意力机制体现神经形态原理，并综述了从利用并行卷积处理的代理梯度到基于强化学习机制的局部学习规则等多种训练方法。

**Conclusion:** 该研究为理解神经形态计算原理在现代AI架构中的体现提供了统一框架，揭示了通过令牌间处理实现高效信息整合的潜力，为设计下一代高能效AI系统指明了方向，同时系统化的训练方法综述为实际应用神经形态AI模型提供了重要参考。
...
### [3] [Three factor delay learning rules for spiking neural networks](https://arxiv.org/abs/2601.00668)
*Luke Vassallo, Nima Taherinejad*

#### 🧩 TL;DR
该研究提出了一种在线学习脉冲神经网络中突触和轴突延迟参数的三因子学习规则，通过引入可学习的延迟参数显著提升了SNN在时序任务中的性能，同时大幅降低了模型规模和推理延迟。

---

#### 📘 Detailed Summary
**Motivation:** 脉冲神经网络通常仅学习突触权重参数，对时序模式识别贡献有限，而现有的延迟学习方法依赖大型网络和离线学习，无法在资源受限环境中实现实时操作，因此需要开发适用于在线学习的延迟参数优化方法。

**Method:** 研究在基于LIF的前馈和循环SNN中引入突触和轴突延迟参数，提出三因子学习规则在线同时学习延迟参数，采用平滑高斯代理函数近似脉冲导数计算资格迹，结合自上而下的误差信号确定参数更新。

**Result:** 实验表明引入延迟参数相比仅学习权重的基线准确率提升高达20%，在相似参数数量下联合学习权重和延迟准确率提升达14%；在SHD语音识别数据集上达到与离线反向传播方法相当的准确率，相比最先进方法模型规模减少6.6倍，推理延迟降低67%，仅损失2.4%的分类准确率。

**Conclusion:** 该方法通过在线学习延迟参数显著提升了SNN的时序处理能力，同时大幅降低了模型复杂度和计算需求，有利于设计功耗和面积受限的神经形态处理器，支持设备端学习并降低内存需求。
