...
#### 🧩 TL;DR
本文提出PickStyle框架，通过低秩适配器和上下文-风格分类器无引导机制，在预训练视频扩散模型基础上实现高质量的视频风格迁移，解决了缺乏配对视频监督数据的关键挑战。

---

#### 📘 Detailed Summary
**Motivation:** 视频风格迁移任务面临缺乏配对视频数据进行监督的关键挑战，现有方法难以在保持视频内容一致性的同时有效传递目标风格，需要开发能够利用静态图像监督数据并确保时间一致性的解决方案。

**Method:** 提出PickStyle框架，在预训练视频扩散模型的注意力层插入低秩适配器实现高效的运动风格迁移，通过共享增强构建合成训练视频片段，并引入上下文-风格分类器无引导机制独立控制内容和风格方向。

**Result:** 在多个基准测试中，该方法实现了时间一致性、风格忠实性和内容保持性的视频转换，在定性和定量评估中均优于现有基线方法，展现出卓越的性能表现。

**Conclusion:** 研究表明结合低秩适配器和专门的训练策略可以有效解决视频风格迁移中的数据稀缺问题，上下文-风格分离引导机制为多模态条件生成提供了新的技术路径，具有重要的实际应用价值。
...
#### 🧩 TL;DR
本文提出TRAVL微调方法和ImplausiBench基准，通过改进视频语言模型的轨迹感知注意力机制来增强物理合理性评估能力，为解决视频生成模型违反物理规律的问题提供了统一框架。

---

#### 📘 Detailed Summary
**Motivation:** 现代视频生成模型虽然视觉保真度高，但经常产生违反物理规律的序列，如物体漂浮、瞬移或违背因果关系的形变，目前缺乏定量评估视频物理真实性的可靠方法，现有视频语言模型在识别物理违规方面存在局限。

**Method:** 提出TRAVL微调方法，结合平衡训练数据集和轨迹感知注意力模块来改进视频语言模型的运动编码和判别能力；同时构建ImplausiBench基准，包含300个视频（150个真实、150个生成），消除语言偏见并专注于视觉时序理解。

**Result:** 实验结果表明现有视频语言模型难以识别物理违规，暴露了其在时序和因果推理方面的根本局限性；TRAVL方法显著提升了物理合理性评估性能，性能评估同时采用人工标注和更严格的LLM-as-judge指标。

**Conclusion:** TRAVL和ImplausiBench为探究和改进多模态模型的物理合理性提供了统一框架，揭示了视觉时序理解中一个具有挑战性且未被充分探索的方面，为视频生成模型的物理真实性评估开辟了新方向。
...
#### 🧩 TL;DR
本研究提出了一种通用的语义光谱-空间融合网络（S3FN），通过利用大语言模型生成的类别特定文本描述来增强高光谱图像分类性能，解决了传统单模态方法在有限训练样本和高维数据下的过拟合问题。

---

#### 📘 Detailed Summary
**Motivation:** 高光谱图像分类面临高质量训练样本稀缺和高维光谱数据带来的挑战，导致模型容易过拟合且难以平衡准确性与计算复杂度，同时现有方法大多为单模态，仅依赖光谱-空间数据在高维嵌入空间中学习决策边界。

**Method:** S3FN利用大语言模型为每个类别标签生成全面的文本描述，捕捉其独特特征和光谱行为，然后使用预训练文本编码器（如BERT或RoBERTa）将这些描述嵌入向量空间，提取有意义的标签语义以实现更好的特征-标签对齐。

**Result:** 在三个不同的高光谱基准数据集（Hyperspectral Wood、HyperspectralBlueberries和DeepHS-Fruit）上的评估表明，该方法显著提升了分类性能，证明了文本语义与光谱-空间数据之间的协同效应。

**Conclusion:** 该研究展示了语义增强在高光谱图像分类中的潜力，为开发更先进的语义增强高光谱分类模型开辟了新途径，强调了多模态融合在解决高维数据分类挑战中的重要性。
...
#### 🧩 TL;DR
本文提出了一种轻量级的视觉语言模型遗忘框架CAGUL，通过跨模态注意力引导的视觉令牌转换来防止敏感信息泄露，同时保持参考模型行为，无需修改预训练模型参数或产生重训练成本。

---

#### 📘 Detailed Summary
**Motivation:** 视觉语言模型在训练过程中可能记忆并泄露私有或敏感信息，而现有的机器遗忘方法主要针对语言模型，视觉语言模型由于同时包含视觉和文本上下文而增加了遗忘过程的复杂性，需要专门解决视觉上下文中敏感信息泄露的问题。

**Method:** 提出跨模态注意力引导遗忘框架CAGUL，利用跨模态注意力分析视觉令牌在输出生成中的作用，通过外部模块对低重要性视觉令牌进行编码转换，从而在不修改预训练模型参数的情况下实现有效遗忘。

**Result:** 实验结果表明，该方法在防止信息泄露方面表现优于或与基于微调的基线方法相当，同时能够保持参考模型的行为特征，实现了高效且实用的视觉语言模型遗忘解决方案。

**Conclusion:** CAGUL框架为视觉语言模型提供了一种轻量级且高效的遗忘机制，通过利用跨模态注意力指导视觉令牌转换，既解决了敏感信息泄露问题，又避免了传统微调方法的高计算成本和模型参数修改，具有重要的实际应用价值。
...
#### 🧩 TL;DR
本文提出了Rectified-CFG++，一种用于整流流扩散模型的自适应预测器-校正器引导方法，通过几何感知的条件规则解决了标准CFG在整流流模型中引起的离流形漂移问题，在多个大规模文本到图像模型上实现了稳定且优越的性能。

---

#### 📘 Detailed Summary
**Motivation:** 标准无分类器引导在整流流扩散模型中应用时会引起严重的离流形漂移，导致视觉伪影、文本不对齐和脆弱行为，这限制了整流流模型在文本条件生成任务中的实际应用效果和稳定性。

**Method:** 提出自适应预测器-校正器引导方法，每个推理步骤首先执行条件整流流更新将样本锚定在学习的传输路径附近，然后应用加权条件校正，在条件和无条件速度场之间进行插值，确保生成轨迹保持在数据流形的有界管状邻域内。

**Result:** 在Flux、Stable Diffusion 3/3.5、Lumina等大规模文本到图像模型上的广泛实验表明，Rectified-CFG++在MS-COCO、LAION-Aesthetic和T2I-CompBench等基准数据集上持续优于标准CFG方法。

**Conclusion:** 该方法证明了整流流模型与几何感知引导策略的有效结合，确保了生成过程的边际一致性和轨迹稳定性，为扩散模型的高效确定性推理提供了新的解决方案，并在广泛的引导强度范围内保持鲁棒性能。
...
#### 🧩 TL;DR
本文提出PIT-QMM，一种新颖的大型多模态模型，用于无参考点云质量评估，通过端到端融合文本、图像和点云数据，在多个基准测试中显著优于现有最先进方法。

---

#### 📘 Detailed Summary
**Motivation:** 当前大型多模态模型在图像和视频质量评估领域已取得显著进展，但在3D资产质量评估领域尚未充分探索，特别是无参考点云质量评估中缺乏能够充分利用多模态互补信息的端到端解决方案。

**Method:** 提出PIT-QMM模型，通过观察发现文本描述、2D投影和3D点云视图为点云质量提供互补信息，构建能够端到端处理文本、图像和点云数据以预测质量分数的新型大型多模态架构。

**Result:** 在流行基准测试上的广泛实验表明，所提方法以显著优势超越现有最先进技术，且训练迭代次数更少，同时框架还支持失真定位和识别功能。

**Conclusion:** 该研究为点云质量评估开辟了新途径，通过多模态融合实现了更准确的评估性能，同时增强模型可解释性和交互性，为3D内容质量分析提供了新的技术方向。
...
#### 🧩 TL;DR
本文提出了一种新颖的弱到强相互学习框架MLH，通过将成对哈希分支的局部相似性知识迁移到中心哈希分支，有效解决了中心哈希方法在建模全局结构时忽略局部相似性信息的问题。

---

#### 📘 Detailed Summary
**Motivation:** 中心哈希方法虽然在捕获全局数据分布方面表现出色，但在建模全局结构时往往忽略了重要的局部相似性信息，这限制了其性能的进一步提升。

**Method:** MLH框架包含一个强大的中心哈希分支和一个较弱的成对哈希分支，通过迭代式相互学习过程实现知识迁移，并引入了基于混合专家的哈希专家混合模块来促进跨分支的有效交互。

**Result:** 在多个基准数据集上的广泛实验表明，MLH方法在图像检索任务中持续优于现有的最先进哈希方法，取得了显著的性能提升。

**Conclusion:** 该研究证明了弱到强相互学习框架在哈希学习中的有效性，通过结合全局结构和局部相似性的优势，为深度哈希方法的发展提供了新的方向。
...
#### 🧩 TL;DR
本文提出了地理时序推理基准GTR-Bench，用于评估视觉语言模型在大规模摄像头网络中移动目标的地理时空推理能力，揭示了当前模型在地理时序推理方面的三大主要缺陷。

---

#### 📘 Detailed Summary
**Motivation:** 现有时空基准主要关注以图像/视频为背景的自中心视角推理或以图形为背景的地理视角推理，缺乏同时利用图像/视频和图形背景评估视觉语言模型地理时空智能的基准，而这对交通管理和应急响应等领域至关重要。

**Method:** 作者构建了GTR-Bench基准，该基准要求模型在地图和视频之间进行多视角切换，对具有非重叠视野的多个视频进行联合推理，并在视频背景未观察到的时空区域进行推理。

**Result:** 对10多个流行视觉语言模型的评估显示，即使最佳专有模型Gemini-2.5-Pro（34.9%）也显著落后于人类表现（78.61%），并揭示了模型在时空上下文利用不平衡、时序预测能力弱以及地图与多视角视频输入理解对齐能力不足三大缺陷。

**Conclusion:** GTR-Bench为时空智能研究提供了宝贵见解和新机遇，揭示了当前视觉语言模型在地理时序推理方面的核心局限性，为未来模型改进指明了方向。
...
#### 🧩 TL;DR
本文提出XYZCylinder，一种基于统一圆柱体提升方法的前馈模型，通过统一相机建模和混合表示设计，解决了驾驶场景重建中固定视图变换泛化能力不足和稀疏视图重建精度受限的问题。

---

#### 📘 Detailed Summary
**Motivation:** 现有前馈重建范式在驾驶场景重建中存在两个主要限制：固定视图变换在相机配置变化时失效，限制了不同驾驶场景间的泛化能力；360°全景稀疏视图间重叠区域小且驾驶场景复杂，增加了学习难度并降低了重建精度。

**Method:** 提出统一圆柱体相机建模策略避免学习视点依赖的空间对应关系，通过可调参数统一不同相机配置；设计基于圆柱平面特征组的混合表示，包含多个专用模块将2D图像特征提升到3D空间。

**Result:** 实验结果表明XYZCylinder在不同评估设置下均达到最先进性能，并能以零样本方式泛化到其他驾驶场景。

**Conclusion:** 该研究证明了统一相机建模和混合表示在提升驾驶场景重建泛化能力和精度方面的有效性，为复杂动态环境下的三维重建提供了新思路。
...
#### 🧩 TL;DR
本文提出MARC方法，通过结构化检索和强化学习蒸馏实现视频令牌压缩，在仅使用一帧令牌的情况下达到接近基准性能，显著降低计算开销。

---

#### 📘 Detailed Summary
**Motivation:** 视觉语言模型从图像扩展到视频时面临高帧率和长持续时间带来的沉重计算成本挑战，现有免训练令牌压缩方法会导致信息丢失和性能下降。

**Method:** 提出MARC框架，采用检索-压缩策略，包含视觉记忆检索器选择关键片段和压缩组相对策略优化框架，将教师模型的推理能力蒸馏到学生模型。

**Result:** 在六个视频基准测试中，MARC仅使用一帧令牌即可达到接近基线准确率，视觉令牌减少95%，GPU内存降低72%，延迟减少23.9%。

**Conclusion:** 该方法展示了在资源受限环境下实现高效实时视频理解的潜力，适用于视频问答、监控和自动驾驶等应用场景。
...
#### 🧩 TL;DR
本文提出了TTOM（测试时优化与记忆）框架，一种无需训练的方法，通过在推理过程中优化新参数并利用参数化记忆机制，显著提升了视频基础模型在组合场景下的文本-图像对齐能力。

---

#### 📘 Detailed Summary
**Motivation:** 视频基础模型在视觉生成方面表现出色，但在组合场景（如运动、数量关系和空间关系）中存在显著不足，导致文本与生成视频内容之间的对齐效果不佳，这限制了模型在复杂场景下的实际应用价值。

**Method:** TTOM框架采用测试时优化策略，通过通用布局-注意力目标引导新参数的集成与优化，而非直接干预潜在空间或注意力机制；同时引入参数化记忆机制，在流式视频生成设置中维护历史优化上下文，支持插入、读取、更新和删除等灵活操作。

**Result:** 在T2V-CompBench和Vbench基准测试上的实验结果表明，TTOM能够有效解耦组合性世界知识，展现出强大的可迁移性和泛化能力，显著提升了组合视频生成的跨模态对齐效果。

**Conclusion:** TTOM框架不仅解决了视频基础模型在组合场景中的对齐问题，还证明了其在无需额外训练的情况下实现高效、可扩展的组合视频生成的可行性，为实时跨模态对齐提供了实用的解决方案。
...
#### 🧩 TL;DR
本文提出了CIR-CoT，这是首个集成显式思维链推理的端到端检索导向多模态大语言模型，通过生成可解释的推理链来增强跨模态交互理解，在提升检索准确性的同时实现决策过程透明化。

---

#### 📘 Detailed Summary
**Motivation:** 当前基于视觉语言模型和多模态大语言模型的组合图像检索方法主要作为黑盒运行，这种不透明性不仅阻碍用户理解检索原理，还限制了模型遵循复杂细粒度指令的能力，需要开发能够提供可解释推理的检索系统。

**Method:** 提出CIR-CoT模型，通过强制模型先生成可解释的推理链来增强跨模态交互捕获能力；采用三阶段过程创建结构化思维链标注，包括描述、推理和结论；对模型进行微调以生成结构化输出，并将最终检索意图编码到专用嵌入中。

**Result:** CIR-CoT在领域内数据集（FashionIQ、CIRR）上实现了高度竞争力的性能，并在领域外CIRCO数据集上表现出显著的泛化能力，为更有效和可信的检索系统开辟了新路径。

**Conclusion:** 该研究证明了集成显式思维链推理能够显著提升组合图像检索的性能和可解释性，结构化推理标注的创建为多模态推理任务提供了新的数据范式，推动了检索系统向更透明和可信的方向发展。
...
#### 🧩 TL;DR
本文提出了DarkHash，首个针对深度哈希模型的无数据后门攻击方法，通过设计具有双重语义引导的影子后门攻击框架，在无需访问训练数据的情况下实现高效后门植入，同时保持原始检索精度。

---

#### 📘 Detailed Summary
**Motivation:** 现有深度哈希模型后门攻击方法需要访问训练数据集进行后门植入，但在现实世界中由于隐私保护和知识产权考虑，获取此类数据往往被禁止，因此如何在无需训练数据的情况下向深度哈希模型嵌入后门，同时保持原始任务检索精度，成为一个新颖且具有挑战性的问题。

**Method:** 设计了一种新颖的影子后门攻击框架，采用双重语义引导机制，通过仅微调受害者模型的特定层并使用代理数据集来嵌入后门功能并保持原始检索精度；利用个体样本与其邻居之间的关系来增强训练过程中的后门攻击，通过设计拓扑对齐损失，优化个体和相邻中毒样本朝向目标样本，进一步提升攻击能力。

**Result:** 在四个图像数据集、五种模型架构和两种哈希方法上的实验结果表明，DarkHash具有高度有效性，优于现有的最先进后门攻击方法；防御实验显示DarkHash能够抵御现有的主流后门防御方法。

**Conclusion:** 该研究证明了在无需访问原始训练数据的情况下对深度哈希模型进行有效后门攻击的可行性，提出了双重语义引导和拓扑对齐损失等创新技术，为深度哈希模型的安全性和鲁棒性研究提供了重要启示，同时也凸显了无数据后门攻击在现实场景中的潜在威胁。
...
#### 🧩 TL;DR
本文提出了一种名为时间条件注意力锐化（TCAS）的方法，通过增强跨模态注意力头的时间分辨能力来解决视频语言模型中存在的响应逻辑不一致问题，显著提升了模型的时间逻辑一致性和时间理解能力。

---

#### 📘 Detailed Summary
**Motivation:** 大型语言模型经常生成自相矛盾的输出，严重影响其可靠性并阻碍实际应用。在视频语言模型中，这种现象表现为模型无法基于其基础输出对重述问题提供逻辑一致的响应，但其根本原因尚未得到充分探索。

**Method:** 本文采用可解释性驱动的方法分析、统计总结并干预该现象的可能因素，提出了时间条件注意力锐化（TCAS）方法，该方法基于注意力差异构建增强目标，通过增强模型的时间分辨能力来改善其时间理解逻辑一致性。

**Result:** 实验结果表明，该方法显著提升了视频语言模型的时间逻辑一致性。进一步的可解释性分析显示，该方法确实改善了注意力头的时间区分能力，验证了研究结论。此外，该方法在通用视频时间定位任务中实现了性能提升。

**Conclusion:** 研究表明时间逻辑一致性是时间理解的关键瓶颈，通过增强一致性可以推动视频时间理解的显著进展。该方法不仅解决了响应不一致问题，还提升了模型在时间相关任务中的整体表现。
...
#### 🧩 TL;DR
本文提出了UniMMVSR，这是首个统一的多模态生成式视频超分辨率框架，能够整合文本、图像和视频等多种模态条件，显著提升了视频生成的质量和条件一致性。
...
**Method:** UniMMVSR采用潜在视频扩散模型，系统探索了条件注入策略、训练方案和数据混合技术，设计了针对不同模态条件的数据构建和利用方法，使模型能够精确利用所有条件类型。

**Result:** 实验表明UniMMVSR显著优于现有方法，生成的视频具有更丰富的细节和更高的多模态条件一致性，并验证了与基础模型结合实现4K视频多模态引导生成的可行性。

**Conclusion:** 该研究证明了多模态条件在视频超分辨率中的重要性，为高质量视频生成提供了新的技术路径，突破了现有技术在4K视频生成方面的限制。
...
#### 🧩 TL;DR
本文提出了MURE框架，通过多模态交错文本-图像思维链将视觉编辑过程从纯文本推理转向视觉化推理，并引入多模态深度置信度机制来减少幻觉问题，显著提升了复杂图像编辑任务的精度和保真度。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于自然语言的图像编辑方法在处理复杂对象交叉和细粒度空间关系时存在困难，主要原因是缺乏显式推理过程。纯文本思维链或仅添加坐标信息的思维链在表示复杂视觉布局和指导像素级细节生成方面存在根本性限制，无法提供必要的视觉线索。

**Method:** 提出了多模态推理编辑框架，采用原生多模态交错文本-图像思维链，在推理链的每一步中文本描述后跟随相应的视觉提示。引入了多模态深度置信度推理范式，通过奖励模型的深度置信度分数修剪低质量分支，确保模型始终沿着高质量轨迹进行编辑。该方法将复杂编辑任务分解为相互依赖的子任务。

**Result:** 在三个图像编辑基准测试中取得了显著改进，通过分解复杂任务实现了每个阶段的更高精度，生成了高保真度的编辑结果。发布了首个CoT-Edit-14K数据集，包含14K个高质量编辑示例。

**Conclusion:** 该研究证明了从纯文本推理转向多模态交错推理对于复杂视觉编辑任务的重要性，多模态深度置信度机制有效缓解了大语言模型的幻觉问题。提出的框架为视觉编辑任务提供了更精确和可控的解决方案，定义了交错文本-图像链的制定标准，为未来多模态推理研究奠定了基础。
...
#### 🧩 TL;DR
本文提出InstructUDrag框架，将文本指令与物体拖拽相结合，解决了现有文本编辑方法定位不精确和拖拽方法仅限于静态重定位的问题，实现了同时进行物体拖拽和基于文本的图像编辑。

---

#### 📘 Detailed Summary
**Motivation:** 当前文本到图像扩散模型在图像编辑中存在两个主要限制：基于文本的方法难以实现精确的物体定位，而物体拖拽方法仅限于静态重定位，无法同时进行语义编辑。

**Method:** 提出InstructUDrag框架，将物体拖拽视为图像重建过程，分为两个协同分支：移动重建分支使用基于能量的梯度引导精确移动物体并优化交叉注意力图，文本驱动编辑分支共享梯度信号确保变换一致性，同时采用DDPM反演和噪声图先验信息注入来保持移动物体的结构完整性。

**Result:** 大量实验表明，InstructUDrag实现了灵活且高保真的图像编辑，在物体重定位精度和图像内容语义控制方面均表现出色。

**Conclusion:** 该研究证明了文本指令与物体拖拽相结合的有效性，为扩散模型提供了更精细的图像编辑控制能力，在物体定位精度和语义属性编辑方面取得了显著进展。
...
### [18] [A Multimodal Depth-Aware Method For Embodied Reference Understanding](https://arxiv.org/abs/2510.08278)
...
#### 📘 Detailed Summary
**Motivation:** 现有开放词汇对象检测方法在存在多个候选对象的模糊场景中经常失败，无法有效处理具身参考理解任务中的歧义性问题，这限制了在复杂或杂乱环境中的实际应用效果。

**Method:** 提出的ERU框架整合了基于LLM的数据增强技术、深度图模态信息以及深度感知决策模块，实现了语言指令和具身线索的鲁棒融合，专门针对复杂环境中的歧义消解进行优化设计。

**Result:** 在两个基准数据集上的实验结果表明，该方法显著优于现有基线方法，在指称目标检测任务中实现了更准确和可靠的性能表现，证明了所提出框架的有效性。

**Conclusion:** 该研究展示了多模态信息融合和深度感知机制在具身参考理解任务中的关键作用，为处理复杂视觉场景中的歧义性问题提供了有效的解决方案，并推动了具身AI系统在实际环境中的应用发展。
...
### [19] [Unlocking 3D Affordance Segmentation with 2D Semantic Knowledge](https://arxiv.org/abs/2510.08316)
*Yu Huang, Zelin Peng, Changsong Wen, Xiaokang Yang, Wei Shen*

#### 🧩 TL;DR
本文提出了一种语义驱动的学习范式，通过跨模态亲和力迁移将大规模2D视觉基础模型的丰富语义知识转移到3D领域，解决了3D功能分割中语义边界模糊的问题，并在标准基准测试中取得了最先进的性能。
...
**Method:** 提出了跨模态亲和力迁移预训练策略，将3D编码器与提升的2D语义对齐，并联合优化重建、亲和力和多样性以产生语义组织的表示；在此基础上设计了跨模态功能分割Transformer，集成多模态提示与CMAT预训练特征来生成精确的提示感知分割图。

**Result:** 在标准基准测试上的广泛实验表明，该框架在3D功能分割任务上建立了新的最先进结果，显著提升了分割精度和语义一致性。

**Conclusion:** 该研究证明了将2D视觉基础模型的语义知识迁移到3D领域的有效性，为3D功能分割提供了一种语义驱动的解决方案，对机器人操作、具身AI和增强现实等应用具有重要意义。
...
### [20] [Evaluating Small Vision-Language Models on Distance-Dependent Traffic Perception](https://arxiv.org/abs/2510.08352)
*Nikos Theodoridis, Tim Brophy, Reenu Mohandas, Ganesh Sistu, Fiachra Collins, Anthony Scanlan, Ciaran Eising*

#### 🧩 TL;DR
本文提出了DTPQA基准测试，这是首个专注于交通场景感知能力的视觉问答基准，通过排除推理问题并添加距离标注来评估小规模视觉语言模型在自动驾驶环境下的感知性能。

---

#### 📘 Detailed Summary
**Motivation:** 当前视觉语言模型在自动驾驶等安全关键应用中缺乏可靠的感知系统评估，特别是对远距离物体的感知能力不足，且现有基准测试往往包含推理任务而无法单独评估纯感知能力。

**Method:** 研究引入了距离标注的交通感知问答基准DTPQA，专门设计仅包含感知问题的视觉问答任务，并重点评估了多个最先进的小规模视觉语言模型在近距离和远距离感知任务上的表现。

**Result:** 实验结果显示，最佳小规模视觉语言模型在DTPQA上的平均准确率约为60%，远低于人类约85%的表现水平，特别是在左右方向区分等基本感知任务上模型表现尤为困难。

**Conclusion:** 研究表明当前小规模视觉语言模型在交通场景感知方面仍存在显著不足，距离实际自动驾驶应用的要求还有较大差距，需要进一步改进模型在基础感知任务上的能力。
...
### [21] [UniVideo: Unified Understanding, Generation, and Editing for Videos](https://arxiv.org/abs/2510.08377)
*Cong Wei, Quande Liu, Zixuan Ye, Qiulin Wang, Xintao Wang, Pengfei Wan, Kun Gai, Wenhu Chen*

#### 🧩 TL;DR
本文提出了UniVideo框架，将统一多模态建模扩展到视频领域，通过双流架构结合多模态大语言模型和视频生成模型，在多种视频生成和编辑任务上达到或超越任务专用模型性能，并支持任务组合和跨任务泛化能力。

---

#### 📘 Detailed Summary
**Motivation:** 当前统一多模态模型在多模态内容生成和编辑方面展现出潜力，但主要局限于图像领域，视频领域的统一建模仍存在明显空白。本研究旨在将统一建模范式扩展到视频领域，解决视频生成和编辑任务中多模态指令理解与视觉一致性的挑战。

**Method:** UniVideo采用双流架构设计，结合多模态大语言模型用于复杂多模态指令的准确理解，以及多模态DiT用于视频生成，确保视觉一致性。该框架将多样化的视频生成和编辑任务统一在单一多模态指令范式下进行联合训练。

**Result:** 大量实验表明，UniVideo在文本/图像到视频生成、上下文视频生成和上下文视频编辑任务中，匹配或超越了当前最先进的任务专用基线模型。该框架展现出任务组合能力和跨任务泛化能力，即使未经自由形式视频编辑的专门训练，也能处理未见过的编辑指令。

**Conclusion:** UniVideo的统一设计实现了两种形式的泛化：支持任务组合能力，以及从大规模图像编辑数据向视频编辑任务的迁移能力。该框架还支持基于视觉提示的视频生成，为未来视频多模态研究提供了重要基础，作者将发布模型和代码以促进后续研究。
...
### [22] [The Visual Iconicity Challenge: Evaluating Vision-Language Models on Sign Language Form-Meaning Mapping](https://arxiv.org/abs/2510.08482)
*Onur Keleş, Aslı Özyürek, Gerardo Ortega, Kadir Gökgö, Esam Ghaleb*

#### 🧩 TL;DR
本研究引入视觉象似性挑战基准，评估了13个最先进的视觉语言模型在手语象似性理解任务上的表现，发现模型在语音形式预测和透明度任务上低于人类基准，但模型对语音形式的敏感度与人类象似性判断存在相关性。

---

#### 📘 Detailed Summary
**Motivation:** 手语中普遍存在的象似性（语言形式与意义之间的相似性）为视觉基础提供了自然测试平台，但现有视觉语言模型面临从动态人体运动而非静态上下文中恢复这种基本映射关系的挑战。

**Method:** 研究引入视觉象似性挑战基准，将心理语言学测量方法应用于视频数据，评估模型在三个任务上的表现：语音符号形式预测（手形、位置）、透明度（从视觉形式推断意义）和分级象似性评分，在零样本和少样本设置下测试了13个最先进的视觉语言模型。

**Result:** 在语音形式预测任务中，模型能够恢复部分手形和位置细节但仍低于人类表现；在透明度任务中，模型表现远低于人类基准；只有顶级模型与人类象似性评分呈现中等相关性，且语音形式预测能力更强的模型与人类象似性判断相关性更高。

**Conclusion:** 研究验证了这些诊断任务的有效性，表明模型对视觉基础结构具有共享敏感性，强调了将人类中心信号和具身学习方法纳入多模态模型以改进视觉基础建模的重要性。
...
### [23] [VideoVerse: How Far is Your T2V Generator from a World Model?](https://arxiv.org/abs/2510.08398)
*Zeqing Wang, Xinyu Wei, Bairui Li, Zhen Guo, Jinrui Zhang, Hongyang Wei, Keze Wang, Lei Zhang*

#### 🧩 TL;DR
本文提出了VideoVerse基准测试，旨在评估文本到视频生成模型在理解复杂时间因果关系和世界知识方面的能力，填补了现有评估体系在事件级时间因果性和世界知识系统评估方面的不足。

---

#### 📘 Detailed Summary
**Motivation:** 现有文本到视频生成技术的快速发展使得传统基准测试日益不足，主要问题包括现有评估维度无法区分最先进模型、事件级时间因果性评估严重不足、以及缺乏对世界知识的系统性评估，而这些能力对于构建世界模型至关重要。

**Method:** 研究团队收集了跨多个领域的代表性视频，提取具有内在时间因果关系的事件级描述，并通过独立标注者将其重写为文本到视频提示，设计了包含十个评估维度的二元评估问题套件，并开发了基于现代视觉语言模型的人类偏好对齐的问答评估流程。

**Result:** VideoVerse基准包含300个精心策划的提示，涉及815个事件和793个二元评估问题，通过对最先进的开源和闭源文本到视频模型进行系统性评估，深入分析了当前文本到视频生成器与世界模型之间的差距。

**Conclusion:** 该研究揭示了当前文本到视频生成模型在理解复杂时间因果关系和世界知识方面仍存在显著不足，为未来模型开发提供了重要的评估框架和方向指导，强调了事件级时间理解能力对于构建真正世界模型的关键重要性。
...
### [24] [To Sink or Not to Sink: Visual Information Pathways in Large Vision-Language Models](https://arxiv.org/abs/2510.08510)
*Jiayun Luo, Wan-Cyuan Fan, Lyuyang Wang, Xiangteng He, Tanzila Rahman, Purang Abolmaesumi, Leonid Sigal*

#### 🧩 TL;DR
本研究揭示了ViT注意力汇（attention sinks）在大型视觉语言模型中的关键作用，提出通过训练无关和训练相关方法来显式利用这些高范数视觉标记，从而显著提升多种LVLM架构在视觉推理任务上的性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有研究主要关注LLM内部的注意力汇问题，而忽视了视觉编码器中高范数视觉标记的重要作用，这些ViT注意力汇虽然包含高层次语义概念，但在现有LVLM架构中往往被忽略，导致视觉信号从ViT到LLM的传播效率不足。

**Method:** 通过定性和定量分析ViT注意力汇中的信息，提出训练无关和训练相关两种方法来更好地利用这些标记，包括改进LLM对这些信息的解释方式，并评估其利用程度。

**Result:** 实验表明显式利用ViT注意力汇标记能够在多种LVLM架构和视觉推理任务上带来显著性能提升，验证了这些标记在增强视觉理解与推理方面的巨大潜力。

**Conclusion:** ViT注意力汇是LVLM中未被充分挖掘的重要资源，通过系统性地利用这些高语义视觉标记可以显著增强模型的视觉推理能力，为未来LVLM架构设计提供了新的优化方向。
...
### [25] [Large Scale Diffusion Distillation via Score-Regularized Continuous-Time Consistency](https://arxiv.org/abs/2510.08431)
*Kaiwen Zheng, Yuji Wang, Qianli Ma, Huayu Chen, Jintao Zhang, Yogesh Balaji, Jianfei Chen, Ming-Yu Liu, Jun Zhu, Qinsheng Zhang*

#### 🧩 TL;DR
本研究首次将连续时间一致性蒸馏扩展到通用应用级图像和视频扩散模型，提出得分正则化连续时间一致性模型(rCM)，通过集成得分蒸馏作为长跳跃正则器，在保持高生成多样性的同时显著提升视觉质量。

---

#### 📘 Detailed Summary
**Motivation:** 尽管连续时间一致性模型(sCM)在学术规模扩散加速方面具有理论优势和实证效果，但其在大规模文本到图像和视频任务中的适用性仍不明确，主要面临雅可比向量积计算的基础设施挑战以及标准评估基准的局限性。

**Method:** 开发了并行兼容的FlashAttention-2 JVP核，支持超过100亿参数模型和高维视频任务的sCM训练；提出得分正则化连续时间一致性模型(rCM)，通过集成得分蒸馏作为长跳跃正则器，将反向散度的模式寻求特性补充到sCM的前向散度目标中。
...
**Conclusion:** rCM作为一个实用且理论基础的框架，通过解决sCM在精细细节生成中的质量限制问题，为推进大规模扩散蒸馏提供了有效解决方案，无需GAN调优或大量超参数搜索即可实现高质量高多样性生成。
...
### [26] [SpatialLadder: Progressive Training for Spatial Reasoning in Vision-Language Models](https://arxiv.org/abs/2510.08531)
*Hongxing Li, Dingming Li, Zixuan Wang, Yuchen Yan, Hang Wu, Wenqi Zhang, Yongliang Shen, Weiming Lu, Jun Xiao, Yueting Zhuang*

#### 🧩 TL;DR
本文提出了一种渐进式训练框架SpatialLadder，通过构建包含26,610个样本的多模态数据集SpatialLadder-26k，采用三阶段训练方法从空间感知到复杂推理逐步建立空间智能，在空间推理基准测试中实现了23.4%的平均性能提升。

---

#### 📘 Detailed Summary
**Motivation:** 当前视觉语言模型在空间推理方面存在根本性挑战，主要问题在于现有方法试图直接学习空间推理而缺乏建立感知与理解的层次化基础，这导致模型难以实现稳健的空间推理性能。

**Method:** 提出了三阶段渐进式训练框架：首先通过目标定位建立空间感知能力，然后通过多维度空间任务发展空间理解，最后通过带可验证奖励的强化学习加强复杂推理；基于构建的SpatialLadder-26k多模态数据集，该数据集包含26,610个样本，涵盖目标定位、单图像、多视角和视频空间推理任务。

**Result:** SpatialLadder模型在空间推理基准测试中实现了23.4%的平均性能提升，超越GPT-4o 20.8%和Gemini-2.0-Flash 10.1%，在领域外基准测试中保持7.2%的泛化性能提升，展示了稳健的空间智能。

**Conclusion:** 研究表明从感知到推理的渐进式训练对于建立稳健的空间智能至关重要，该方法为视觉语言模型的空间推理能力提供了系统性的解决方案，并验证了层次化学习框架的有效性。
...
### [27] [Gaze on the Prize: Shaping Visual Attention with Return-Guided Contrastive Learning](https://arxiv.org/abs/2510.08442)
*Andrew Lee, Ian Chuang, Dechen Gao, Kai Fukazawa, Iman Soltani*

#### 🧩 TL;DR
本文提出Gaze on the Prize框架，通过可学习的中央凹注意力机制增强视觉强化学习，利用回报差异指导注意力聚焦任务相关特征，在ManiSkill3基准测试中实现了最高2.4倍的样本效率提升。

---

#### 📘 Detailed Summary
**Motivation:** 视觉强化学习代理需要处理高维图像数据，但只有少量像素与任务相关，导致代理浪费探索和计算资源在无关特征上，造成样本效率低下和学习不稳定。

**Method:** 该方法引入可学习的中央凹注意力机制，通过基于回报差异的自监督信号指导注意力学习，采用回报引导的对比学习将相似视觉表示按回报差异分组为正负样本，构建对比三元组训练注意力机制产生可区分的表示。

**Result:** 在ManiSkill3基准测试的多个操作任务中，该方法实现了最高2.4倍的样本效率提升，并能解决基线方法无法学习的任务，且无需修改底层算法或超参数。

**Conclusion:** 研究表明回报差异能够有效揭示任务相关特征，通过对比学习引导注意力机制可以显著提高视觉强化学习的样本效率和稳定性，为处理高维视觉输入提供了新的解决方案。
...
### [28] [VideoNorms: Benchmarking Cultural Awareness of Video Language Models](https://arxiv.org/abs/2510.08543)
*Nikhil Reddy Varimalla, Yunfei Xu, Arkadiy Saakyan, Meng Fan Wang, Smaranda Muresan*

#### 🧩 TL;DR
本文提出了VideoNorms基准数据集，包含1000多个视频片段与社会文化规范配对，用于评估视频大语言模型的文化意识，揭示了模型在文化理解方面的系统性缺陷。
...
**Method:** 采用人机协作框架构建数据集，其中使用基于理论指导提示的教师模型提供候选标注，由训练有素的人类专家验证和修正标注，构建了基于言语行为理论的社会文化规范标注。

**Result:** 基准测试显示多个共同趋势：模型在规范违反检测上表现更差；对中国文化的理解不如美国文化；提供非语言证据比语言证据更困难；在正式非幽默语境中表现不如人类。

**Conclusion:** 研究强调了文化基础视频语言模型训练的必要性，该基准和框架为填补这一空白提供了起点，揭示了当前模型在跨文化理解方面的系统性局限性。
...
### [29] [Video-STAR: Reinforcing Open-Vocabulary Action Recognition with Tools](https://arxiv.org/abs/2510.08480)
...
#### 🧩 TL;DR
本文提出Video-STAR框架，通过上下文子动作分解与工具增强强化学习的协同设计，解决多模态大语言模型在开放词汇动作识别中的语义混淆问题，显著提升细粒度动作区分能力并减少跨模态幻觉。

---

#### 📘 Detailed Summary
**Motivation:** 多模态大语言模型在视觉-文本推理方面展现出巨大潜力，但其对文本中心先验的依赖限制了在开放词汇场景下区分语义相似动作的能力，现有方法将动作视为整体实体而缺乏细粒度分析，导致跨模态幻觉问题。

**Method:** 提出Video-STAR框架，创新性地将动作分解为具有区分性的子动作以实现细粒度匹配，同时动态调用领域特定工具进行跨模态交织，设计分层奖励机制平衡工具使用效率、子动作相关性和推理结构一致性，无需显式监督即可自主利用外部工具优先处理子动作模式。

**Result:** 在HMDB-51、UCF-101、SSv2、Kinetics-400和Kinetics-600数据集上的广泛评估表明，该方法在区分细粒度动作和处理跨模态幻觉方面优于现有方法，实现了最先进的性能，验证了其优秀的鲁棒性和泛化能力。

**Conclusion:** 该研究证明了将动作分解为子动作并结合工具增强强化学习的有效性，能够实现从文本中心推理到视觉基础推理的转变，为开放词汇动作识别提供了新的技术路径，对多模态理解系统的实际应用具有重要启示意义。
...
### [30] [MATRIX: Multimodal Agent Tuning for Robust Tool-Use Reasoning](https://arxiv.org/abs/2510.08567)
*Tajamul Ashraf, Umair Nawaz, Abdelrahman M. Shaker, Rao Anwer, Philip Torr, Fahad Shahbaz Khan, Salman Khan*

#### 🧩 TL;DR
本文提出了一个视觉中心的多模态智能体调优框架，通过自动合成多模态轨迹和生成逐步偏好对，训练VLM控制器实现稳健的工具使用推理，在多个基准测试中超越了开源和闭源VLM模型。

---

#### 📘 Detailed Summary
**Motivation:** 当前视觉语言模型作为控制器访问外部工具进行复杂推理和决策时，受到高质量多模态轨迹稀缺和手动标注成本高昂的限制，需要开发自动化的轨迹合成和偏好学习方法来提升多模态工具使用的有效性。

**Method:** 提出了视觉中心智能体调优框架，首先构建包含28.5K多模态任务和177K验证轨迹的M-TRACE数据集进行模仿学习，然后开发MATRIX Agent控制器进行逐步工具推理，进一步通过Pref-X生成的11K偏好对进行逐步偏好学习优化。

**Result:** 在Agent-X、GTA和GAIA三个基准测试中，MATRIX控制器一致超越了开源和闭源的视觉语言模型，证明了该方法在多模态工具使用方面的可扩展性和有效性。

**Conclusion:** 该研究展示了通过自动轨迹合成和偏好学习能够显著提升多模态智能体的工具使用能力，为构建更强大的多模态推理系统提供了可扩展的数据驱动方法，推动了视觉语言模型在实际应用中的发展。
...
### [31] [InstructX: Towards Unified Visual Editing with MLLM Guidance](https://arxiv.org/abs/2510.08485)
*Chong Mou, Qichao Sun, Yanze Wu, Pengze Zhang, Xinghui Li, Fulong Ye, Songtao Zhao, Qian He*

#### 🧩 TL;DR
本文提出了InstructX框架，通过深入研究多模态大语言模型与扩散模型的集成设计，实现了图像和视频编辑任务的统一建模，并在无需显式监督的情况下展现出视频编辑的涌现能力。

---

#### 📘 Detailed Summary
**Motivation:** 当前多模态大语言模型在视觉理解方面取得显著进展，但将其与扩散模型集成用于编辑任务的研究缺乏深入的设计分析，特别是在视频编辑等复杂任务中仍面临挑战，且图像与视频在统一建模中的协同与差异关系尚未得到系统探索。

**Method:** 提出了InstructX统一框架，通过全面研究MLLM与扩散模型的集成策略，分析图像和视频在统一建模中的协同与差异关系，利用图像数据训练实现视频编辑的涌现能力，并整合模态特定的MLLM特征来统一处理图像和视频编辑任务。

**Result:** 实验表明该方法能够处理广泛的图像和视频编辑任务，在无需显式监督的情况下展现出视频编辑的涌现能力，有效缓解了视频训练数据稀缺的限制，并在多个任务上达到了最先进的性能水平。

**Conclusion:** 研究揭示了图像训练能够自然迁移到视频编辑任务的潜力，为多模态编辑系统提供了统一的建模范式，同时通过模态特定特征的整合实现了跨模态任务的协同优化，为未来多模态编辑研究提供了重要启示。
...
### [32] [MoA-VR: A Mixture-of-Agents System Towards All-in-One Video Restoration](https://arxiv.org/abs/2510.08508)
...
#### 🧩 TL;DR
本文提出了MoA-VR，首个基于智能体混合的视频修复系统，通过三个协调的智能体模拟人类专家的推理和处理流程，有效处理复杂多样的视频退化问题，在客观指标和感知质量上均优于现有基线方法。

---

#### 📘 Detailed Summary
**Motivation:** 现实世界视频由于不同的采集和传输条件常常遭受复杂的退化问题，如噪声、压缩伪影和低光失真，现有修复方法通常需要专业人员手动选择专用模型或依赖无法泛化处理不同退化的单一架构，存在明显的局限性。

**Method:** 提出MoA-VR系统，包含三个协调智能体：退化识别、路由与修复、修复质量评估；构建大规模高分辨率视频退化识别基准和视觉语言模型驱动的退化识别器；引入基于大语言模型的自适应路由器，通过观察工具使用模式自主学习有效修复策略；构建Res-VQ数据集并设计专门针对修复任务的VLM视频质量评估模型。

**Result:** 大量实验表明，MoA-VR能有效处理多样化和复合退化问题，在客观指标和感知质量方面持续优于现有基线方法，展示了系统在处理复杂视频退化方面的优越性能。

**Conclusion:** 该研究展示了多模态智能和模块化推理在通用视频修复系统中的整合潜力，为构建更智能、自适应的视频处理系统提供了新的技术路径和设计范式。
...
### [33] [SciVideoBench: Benchmarking Scientific Video Reasoning in Large Multimodal Models](https://arxiv.org/abs/2510.08559)
*Andong Deng, Taojiannan Yang, Shoubin Yu, Lincoln Spencer, Mohit Bansal, Chen Chen, Serena Yeung-Levy, Xiaohan Wang*

#### 🧩 TL;DR
本文提出了SciVideoBench，一个专门用于评估科学视频推理能力的严格基准，包含1000个来自前沿科学实验视频的多选题，揭示了当前大型多模态模型在复杂科学视频推理方面的显著性能缺陷。

---

#### 📘 Detailed Summary
**Motivation:** 当前视频基准主要针对通用场景，依赖感知识别而推理任务相对简单，导致性能饱和，无法有效评估高级多模态认知技能，特别是在科学领域中的复杂视频推理仍然是一个具有挑战性的前沿问题。

**Method:** 构建了包含1000个精心设计多选题的SciVideoBench基准，这些题目源自涵盖25+专业学科的前沿科学实验视频，并通过半自动系统验证，每个问题都需要复杂的领域知识、精确的时空感知和精细的逻辑推理。

**Result:** 评估显示包括Gemini 2.5 Pro和Qwen2.5-VL在内的最先进专有和开源大型多模态模型都存在显著的性能缺陷，表明在视频推理能力方面仍有巨大的提升空间。

**Conclusion:** 该研究为大型多模态模型的未来发展提供了有价值的见解和明确方向，推动了真正有能力的多模态AI科学助手的演进，通过详细分析推理复杂性和视觉基础等关键因素，为前沿AI在更广泛科学领域的边界推进提供了帮助。
...
#### 🧩 TL;DR
本文针对多模态大语言模型在长链反思推理能力上的不足，提出了MM-HELIX基准测试和自适应混合策略优化方法，显著提升了模型在复杂推理任务上的性能。该方法在Qwen2.5-VL-7B模型上实现了18.6%的准确率提升，并展现出良好的泛化能力。

---

#### 📘 Detailed Summary
**Motivation:** 当前多模态大语言模型虽然在数学和逻辑推理任务上表现出色，但其长链反思推理能力——解决复杂现实问题的关键前提——仍然未被充分探索。现有模型在需要迭代思考和回溯的复杂推理任务上存在显著性能缺陷，这限制了其在真实场景中的应用潜力。

**Method:** 研究首先构建了MM-HELIX多模态基准测试，包含1,260个需要迭代思考和回溯的挑战性任务样本。随后开发了步骤引导响应生成流水线来创建MM-HELIX-100K大规模数据集，包含10万个高质量反思推理轨迹。针对标准强化学习在复杂任务中因稀疏奖励信号和灾难性遗忘而失效的问题，提出了自适应混合策略优化方法，将离线监督和在线优化动态统一到单阶段训练中。

**Result:** 在MM-HELIX基准测试上，现有MLLMs表现出显著的性能缺陷。应用所提方法后，Qwen2.5-VL-7B模型在基准测试上实现了+18.6%的准确率提升，并在通用数学和逻辑任务上展现出+5.7%的平均性能增益，证明了方法的强泛化能力。

**Conclusion:** 研究表明多模态大语言模型的反思推理能力可以通过有效学习得到显著提升和泛化。自适应混合策略优化方法为解决复杂推理任务中的训练挑战提供了有效方案，为开发更强大的多模态大语言模型开辟了新途径。该方法使模型能够在奖励稀疏时从专家数据中学习，并在熟练后进行独立探索。
...
#### 🧩 TL;DR
本文提出了一个支持多模态控制的视频插帧框架，通过将运动控制映射为统一的点基表示，并采用双分支架构分别处理内容和运动控制，实现了灵活、精确的视频过渡生成。

---

#### 📘 Detailed Summary
**Motivation:** 现有视频插帧方法无法生成大规模、复杂或精细的运动，且缺乏对中间帧细节的精细控制，难以满足用户多样化的创作意图，导致与创意构思不一致的问题。

**Method:** 采用Diffusion Transformer作为视频生成模型，将所有运动控制映射为统一的稀疏点基表示作为视频/噪声输入，将内容控制和运动控制分离为两个分支进行特征编码，并采用分阶段训练策略确保多模态控制的平滑学习。

**Result:** 广泛的定性和定量实验表明，多模态控制能够实现更动态、可定制且上下文准确的视觉叙事，在视频插帧质量和控制精度方面表现出色。

**Conclusion:** 该研究证明了多模态控制在视频插帧中的有效性，为视频编辑和长视频合成提供了更灵活、精确的创作工具，平衡了灵活性、易用性和精细控制的需求。
...
#### 🧩 TL;DR
本文提出了一种名为NaViL的原生多模态大语言模型，通过端到端训练系统研究了MLLM的设计空间和扩展特性，在数据约束条件下实现了视觉编码器与语言模型之间的正向扩展关系。

---

#### 📘 Detailed Summary
**Motivation:** 现有MLLM通常采用组合式训练范式，将预训练的视觉编码器与预训练的语言模型通过连续多模态预训练连接，但这种分离训练方式难以探索多模态扩展特性，因此需要研究原生端到端训练的MLLM设计空间和扩展规律。

**Method:** 在数据约束的实际设置下，系统研究了MLLM的各种设计选择，获得了在性能和训练成本之间最佳平衡的元架构，并进一步探索了原生MLLM的扩展特性，提出了结合简单且成本有效配方的NaViL原生MLLM。

**Result:** 在14个多模态基准测试上的实验结果表明，NaViL相对于现有MLLM具有竞争优势，同时揭示了视觉编码器与语言模型之间正向相关的扩展关系，为原生MLLM的未来研究提供了深入见解。

**Conclusion:** 研究证实了原生MLLM端到端训练的可行性，发现了视觉组件与语言组件之间的正向扩展规律，提出的NaViL模型不仅性能优异，更为后续原生多模态大模型的研究提供了重要的设计指导和理论基础。
...
#### 🧩 TL;DR
本研究提出了两种成本高效的评估方法：多标准提示和领域自适应迁移学习，成功提升了小型视觉语言模型在图表理解任务中作为自动评判器的性能，使2B参数的ChartJudge模型能够有效替代7B模型进行图表推理评估。

---

#### 📘 Detailed Summary
**Motivation:** 当前大型视觉语言模型在图表理解任务中作为自动评判器表现出色，但小型模型（≤2B参数）性能仍然较差，限制了在资源受限环境中的实际应用，因此需要开发成本高效的评估方法来解决这一限制。

**Method:** 提出了两种核心方法：多标准提示将多个评估标准整合到单个查询中，以及领域自适应迁移学习，通过在图表数据集上使用合成判断对2B参数的LVLM进行微调，创建专门的ChartJudge模型。

**Result:** 实验表明多标准提示暴露了7B模型的鲁棒性差距，导致包括LLaVA-Critic在内的专业LVLM评判器性能大幅下降，同时ChartJudge能够有效在不同数据集间迁移知识，成为更专业的模型。
...
#### 🧩 TL;DR
本文提出了ToolExpander框架，通过动态多轮硬采样和自示例思维两项关键创新，解决了小规模LLM在GRPO训练中响应不准确和训练崩溃的问题，显著提升了工具使用能力和训练稳定性。
...
**Method:** ToolExpander框架包含两项关键创新：动态多轮硬采样在训练期间动态替换困难样本为高质量少样本演示，并结合指数学习率衰减策略缓解振荡；自示例思维是增强的GRPO框架，消除了KL散度并整合调整的裁剪系数，通过最小额外奖励（0.01）鼓励模型自主生成和分析少样本示例。

**Result:** 实验结果表明，ToolExpander显著增强了LLM的工具使用能力，特别是在较弱的小规模模型中，同时改善了训练稳定性和整体性能。该方法有效解决了模型响应不准确和训练崩溃的问题。

**Conclusion:** 该研究证明了通过创新的训练策略可以显著提升资源受限LLM的工具使用能力，为小规模模型的有效强化学习训练提供了可行解决方案，具有重要的实际应用价值。
...
#### 🧩 TL;DR
LLM4Cell提出了首个统一的大语言模型在单细胞生物学领域的系统综述，涵盖了58个基础模型和智能体框架，通过整合数据集、模型和评估维度，为语言驱动的单细胞智能研究提供了综合视图。

---

#### 📘 Detailed Summary
**Motivation:** 当前大语言模型和智能体框架在单细胞生物学中的应用仍存在数据模态、架构和评估标准碎片化的问题，缺乏统一的系统性综述来整合这一快速发展领域的研究进展。

**Method:** 研究系统性地分类了58个单细胞研究相关的基础模型和智能体框架，将其划分为基础模型、文本桥接、空间组学、多模态、表观基因组和智能体六大类别，并映射到八个关键分析任务中。

**Result:** 基于40多个公共数据集的分析显示，该研究评估了模型在10个领域维度上的表现，包括生物学基础、多组学对齐、公平性、隐私保护和可解释性，并分析了基准测试的适用性和数据多样性约束。

**Conclusion:** LLM4Cell为语言驱动的单细胞智能研究提供了首个集成视图，指出了在可解释性、标准化和可信模型开发方面存在的开放挑战，为未来研究指明了方向。
...
#### 🧩 TL;DR
本研究提出了CS3-Bench基准测试，揭示了多模态大语言模型在语码转换语音交互中的语言对齐缺陷，并提出基于链式识别和关键词高亮的方法显著提升了模型性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有多模态大语言模型在实现自然单语交互方面已取得进展，但在语言对齐能力方面存在明显不足，特别是在语码转换场景下，模型难以准确理解和生成混合语言的语音内容。

**Method:** 研究团队构建了CS3-Bench基准测试，并提出了数据构建和训练方法，具体采用链式识别来增强理解能力，以及关键词高亮来引导生成过程，从而改善语言对齐性能。

**Result:** 在7个主流模型上的实验显示，在知识密集型问答任务中性能相对下降高达66%，而提出的方法将知识准确率从25.14%提升至46.13%，开放式理解率从64.5%提升至86.5%，并显著减少了第二语言的发音错误。

**Conclusion:** 该研究强调了多模态语音交互系统中语言对齐的重要性，提出的方法为改善语码转换场景下的模型性能提供了有效解决方案，并为未来多语言语音交互系统的开发指明了方向。
...
#### 🧩 TL;DR
本研究展示了大型语言模型在17-18世纪爱沙尼亚语历史词典研究中的应用潜力，包括词典信息半自动丰富、哥特体文字识别和跨源数据集构建，为小语种历史文献数字化提供了高效解决方案。

---

#### 📘 Detailed Summary
**Motivation:** 该研究旨在解决17-18世纪爱沙尼亚语历史词典数字化过程中的关键挑战，包括词典信息不完整、哥特体文字识别困难以及跨源数据整合复杂等问题，这些因素严重制约了小语种历史语言资源的开发利用。

**Method:** 研究采用多模态大型语言模型技术，包括使用Claude 3.7 Sonnet进行词典条目语义丰富化，运用视觉增强LLM对哥特体印刷文本进行零样本识别，并通过重叠切片扫描与双模型协作策略实现结构化数据提取与合并。

**Result:** 实验结果显示，在充分上下文条件下，Claude 3.7 Sonnet能够为81%的词目条目准确提供现代词义和对应形式；在文本识别任务中，零样本方法成功识别并结构化41%的词目条目为无错误JSON格式；Hupel词典数字化项目采用重叠切片技术实现了高效数据提取。

**Conclusion:** 研究表明大型语言模型即使对小语种历史文献处理也具有显著优势，能够大幅节省时间和经济成本，为历史语言学研究和文化遗产数字化提供了新的技术路径，证明了AI技术在低资源语言处理中的可行性和有效性。
...
#### 🧩 TL;DR
本研究提出了一种针对科学图表标题生成的领域特定系统，通过结合上下文理解和作者特定风格适应，能够生成既科学准确又风格忠实于源论文的标题。该系统采用两阶段流水线方法，在SciCap挑战中展示了显著的性能提升。

---

#### 📘 Detailed Summary
**Motivation:** 科学图表标题需要同时具备准确性和风格一致性来传达视觉信息，现有方法在保持作者特定写作风格方面存在不足。本研究旨在解决科学图表标题生成中上下文理解与风格适应性之间的平衡问题，特别是在多类别科学图表场景下。

**Method:** 采用两阶段流水线方法：第一阶段结合上下文过滤、基于DSPy MIPROv2和SIMBA的类别特定提示优化以及标题候选选择；第二阶段应用基于配置文件的少样本提示进行风格精炼，利用LaMP-Cap数据集实现作者特定风格适应。

**Result:** 实验表明类别特定提示方法显著优于零样本和通用优化方法，ROUGE-1召回率提升+8.3%，同时将精度损失限制在-2.8%，BLEU-4减少控制在-10.9%。基于配置文件的风格精炼在BLEU分数上获得40-48%的提升，在ROUGE指标上获得25-27%的提升。

**Conclusion:** 研究表明结合上下文理解与作者特定风格适应能够生成既科学准确又风格忠实的标题，为领域特定的科学内容生成提供了有效框架。该方法强调了在科学交流中保持作者独特写作风格的重要性，并为未来的多模态科学内容生成研究指明了方向。
...
#### 🧩 TL;DR
MUSE提出了一种基于分层记忆模块的经验驱动自进化AI智能体框架，解决了现有LLM智能体在真实世界长视野任务中无法从经验中学习的关键限制，实现了持续学习和自我进化能力。

---

#### 📘 Detailed Summary
**Motivation:** 现有大型语言模型智能体存在关键限制：它们是测试时静态的，无法从经验中学习，缺乏积累知识和持续改进的能力，这阻碍了它们在真实世界长视野任务中的有效部署。

**Method:** MUSE引入了一个以分层记忆模块为核心的经验驱动自进化系统，该框架组织不同层次的经验并利用它们规划执行多应用长视野任务，智能体在每个子任务执行后自主反思其轨迹，将原始轨迹转换为结构化经验并整合回记忆模块。

**Result:** 在长视野生产力基准TAC上，MUSE仅使用轻量级Gemini-2.5 Flash模型就实现了显著超越现有最佳性能的新SOTA，实验表明随着智能体自主积累经验，其任务完成能力持续提升，并展现出强大的持续学习和自进化能力，积累的经验还表现出强泛化特性，能够零样本改进新任务。

**Conclusion:** MUSE建立了一种能够实现真实世界生产力任务自动化的AI智能体新范式，其经验驱动机制使智能体能够超越静态预训练参数实现持续进化，为构建具有长期学习能力的AI系统提供了重要方向。
...
#### 🧩 TL;DR
本综述系统梳理了过程奖励模型（PRMs）的研究进展，旨在解决传统结果奖励模型仅评估最终答案的局限性，通过评估推理过程来提升大语言模型的细粒度推理对齐能力。

---

#### 📘 Detailed Summary
**Motivation:** 尽管大语言模型展现出先进的推理能力，但传统的对齐方法主要依赖结果奖励模型（ORMs），这些模型仅对最终答案进行评判，无法对推理过程进行细粒度评估和指导，过程奖励模型（PRMs）正是为了填补这一研究空白而提出的。

**Method:** 论文系统性地介绍了过程奖励模型的完整构建流程，包括过程数据生成方法、PRMs模型构建技术，以及如何将PRMs应用于测试时扩展和强化学习等场景。

**Result:** 研究总结了PRMs在数学、代码、文本、多模态推理、机器人技术和智能体等多个领域的应用实践，并对新兴的基准测试进行了全面评述，展现了该技术在不同任务中的适用性。

**Conclusion:** 本综述明确了过程奖励模型的设计空间，揭示了当前面临的开放挑战，为未来研究提供了指导方向，推动大语言模型向更细粒度、更鲁棒的推理对齐方向发展。
...
#### 🧩 TL;DR
本文提出DACIP-RC方法，通过阅读理解式持续预训练增强小型LLM在业务对话任务中的领域适应能力，显著提升了零样本泛化性能。

---

#### 📘 Detailed Summary
**Motivation:** 大规模LLM的高推理成本使其部署不切实际，而小型LLM缺乏跨领域的零样本指令跟随能力，传统微调方法又会导致灾难性遗忘，限制了模型对动态用户需求的适应性。

**Method:** 提出领域自适应持续指令预训练方法DACIP-RC，通过阅读理解方式在对话转录本上生成多样化任务指令和响应，替代传统的下一词元预测方法，实现更好的指令泛化能力。

**Result:** 实证评估表明DACIP-RC在多种业务对话任务中显著提升零样本泛化性能，包括会议摘要、行动项生成和通话目的识别等任务。

**Conclusion:** 这是首个将指令预训练应用于业务对话数据的工作，为行业如何利用专有数据集进行领域适应提供了重要见解，展示了持续预训练在增强小型模型适应性方面的潜力。
...
#### 🧩 TL;DR
本文提出ARM2模型，通过强化学习框架和长度感知优化，自适应平衡推理性能与效率，在保持与传统推理模型相当性能的同时平均减少70%以上的token使用量。
...
**Method:** ARM2采用强化学习框架增强长度感知优化，统一平衡多种格式的推理性能与效率，不仅整合视觉理解扩展到多模态应用，还集成可执行代码到推理过程中以减少token成本。

**Result:** 实验表明ARM2在性能上与使用GRPO训练的传统推理模型相当，同时平均减少超过70%的token使用量，广泛分析验证了模型有效性和设计合理性。

**Conclusion:** ARM2为自适应推理提供了通用解决方案，通过多模态集成和代码执行显著提升效率，为未来推理模型的高效优化指明了方向。
...
#### 🧩 TL;DR
本研究提出了一种名为Co⁴的轻量级语言模型，仅包含单层、两个注意力头和800万参数，在近似O(N)计算复杂度下超越了BabyLM挑战赛中的GPT-2和GPT-BERT基线模型，展示了极高的训练效率和样本效率。

---

#### 📘 Detailed Summary
**Motivation:** 当前主流深度学习范式主要依赖深层网络架构和O(N²)计算复杂度，这导致了巨大的计算开销和训练成本，本研究旨在探索更高效的语言模型架构设计，挑战现有的缩放定律和深度网络主导的范式。

**Method:** 采用Co⁴机器架构，仅包含单层Transformer、两个注意力头和800万参数，通过优化计算复杂度至近似O(N)，在仅训练两个周期的情况下与训练十个周期的基线模型进行对比。

**Result:** 在1000万token数据集上，Co⁴实现了数量级更高的训练效率，在BabyLM挑战赛评估中，零样本任务上在7个指标中有5个超越GPT-2、4个超越GPT-BERT，微调任务上在7个任务中有6个超越GPT-2、4个超越GPT-BERT。

**Conclusion:** 这些结果表明需要重新思考当前主流的深度学习范式和相关的缩放定律，轻量级架构在保持性能的同时可以显著提升训练效率，为高效语言模型设计提供了新的方向。
...
#### 🧩 TL;DR
本文提出ARES框架，一种自适应推理的统一开源框架，通过基于任务难度动态分配探索努力来解决多模态大推理模型在简单问题上过度思考而在困难问题上探索不足的问题。

---

#### 📘 Detailed Summary
**Motivation:** 当前多模态大推理模型存在推理努力分配不平衡的问题：在简单问题上产生不必要的冗长推理轨迹，而在困难问题上探索不足导致错过解决方案。这种不平衡限制了模型在实际应用中的效率和性能。

**Method:** ARES采用两阶段训练流程：自适应冷启动阶段通过策划推理轨迹长度与问题难度成比例的多模态和文本数据，使模型具备初步难度感知能力；第二阶段开发自适应熵策略优化，使用高窗口熵令牌作为探索触发器来决定何时探索，并采用具有动态KL控制的分层熵奖励来决定探索程度。

**Result:** 广泛实验表明，ARES在多样化的数学、逻辑和多模态基准测试中实现了优越的性能和推理效率，同时在显著降低推理成本的情况下缩小了与领先商业系统的差距。

**Conclusion:** 该研究证明了基于任务难度动态调整推理探索的有效性，高窗口熵令牌能够可靠捕捉推理关键时刻，为开发更高效的多模态推理系统提供了新方向，并展示了在保持性能的同时大幅降低计算成本的可能性。
...
#### 🧩 TL;DR
本文提出TS-Agent，一种时间序列推理智能体，通过将LLMs擅长的高层推理与时间序列分析工具的专业能力相结合，有效解决了LLMs在时间序列推理任务中的幻觉和知识泄露问题。

---

#### 📘 Detailed Summary
**Motivation:** 大型语言模型在推理和问题解决方面表现出强大能力，但现有研究表明它们在时间序列推理任务中仍存在困难，输出经常受到幻觉或知识泄露的影响，这成为当前研究需要解决的关键问题。

**Method:** TS-Agent采用原子操作符与原始数值序列交互，将统计和结构信息提取委托给时间序列分析工具，同时利用LLMs进行证据收集和逐步推理合成，通过显式证据记录、自我批评机制和最终质量门控实现迭代推理优化。

**Result:** 实验评估表明，TS-Agent在理解基准测试中达到与最先进LLMs相当的性能，在推理任务上实现显著改进，特别是在零样本设置下，现有模型通常依赖记忆而失败的情况下表现突出。

**Conclusion:** 该研究证明了将LLMs的高层推理能力与领域专用工具相结合的有效性，避免了多模态对齐训练，保持了时间序列的原始形式，确保了可解释性和可验证性，为时间序列推理提供了新的解决方案。
...
#### 🧩 TL;DR
本研究评估了多个大型语言模型在理解BPMN业务流程模型、检测语法逻辑错误以及通过自然语言界面进行深度推理的能力，发现未经训练的LLM在零样本设置下能够有效分析业务流程模型并发挥助手作用。

---

#### 📘 Detailed Summary
**Motivation:** 该研究旨在探索大型语言模型在业务流程建模领域的应用潜力，特别是评估它们能否通过对话式交互理解业务流程模型、检测模型中的语法和逻辑错误，并通过自然语言界面进行深度推理，填补了LLM在业务流程分析领域系统评估的研究空白。

**Method:** 研究采用实证分析方法，测试了多个LLM（包括ChatGPT o3模型）在零样本设置下对BPMN流程模型图像的理解能力，通过自然语言查询评估模型在语法、逻辑和语义层面的分析能力，并深入研究了LLM的"思维过程"和深度推理机制。

**Result:** 实验结果表明，未经训练的LLM在零样本设置下能够有效理解BPMN流程模型图像，并在语法、逻辑和语义层面智能地回答相关查询，不同LLM在准确性和有效性方面表现存在差异，同时发现LLM在流程分析和优化中展现出类人化的推理特性。

**Conclusion:** 该研究证实了LLM作为业务流程设计者和用户助手的潜在价值，揭示了LLM在业务流程分析中表现出的类人化特性，为未来将LLM集成到业务流程管理工具中提供了实证基础，并指出了进一步研究LLM深度推理能力的方向。
...
#### 🧩 TL;DR
RECAP是一个用于低资源语言个人身份信息检测的混合框架，结合确定性正则表达式和上下文感知大语言模型，在13个低资源语言环境中显著优于现有方法，提供可扩展的隐私合规解决方案。

---

#### 📘 Detailed Summary
**Motivation:** 该研究旨在解决低资源语言中个人身份信息检测面临的挑战，包括语言多样性问题和标注数据有限性，这些因素阻碍了隐私合规应用的有效部署。

**Method:** RECAP采用混合框架设计，结合确定性正则表达式与上下文感知大语言模型，通过模块化架构支持300多种实体类型而无需重新训练，并采用三阶段精炼流程进行消歧和过滤。

**Result:** 使用nervaluate基准测试表明，该系统在加权F1分数上比微调NER模型高出82%，比零样本LLM高出17%，在13个低资源语言环境中均表现出优越性能。

**Conclusion:** 该研究提供了一个可扩展且适应性强的解决方案，能够有效支持合规导向应用中的个人身份信息检测，解决了低资源语言环境下的实际部署挑战。
...
#### 🧩 TL;DR
本文揭示了当前评估指标系统性低估了前沿AI模型的组合推理能力，提出了组匹配评分和测试时匹配算法，显著提升了模型在组合推理基准上的表现，并在多个数据集上实现了新的最先进结果。

---

#### 📘 Detailed Summary
**Motivation:** 前沿AI模型在组合推理方面表现不佳，现有评估指标系统性低估了模型能力，导致在标准基准测试中模型表现往往接近或低于随机水平，这掩盖了模型真实的组合推理潜力。

**Method:** 提出了组匹配评分方法以更好地利用组结构揭示模型隐藏能力，并设计了测试时匹配算法，这是一种无需外部监督的迭代自改进方法，通过测试时过拟合组匹配将隐藏能力转化为标准评估指标下的更高分数。

**Result:** 该方法使SigLIP-B16超越了所有先前结果和GPT-4，GPT-4.1在Winoground上首次超越估计的人类表现，测试时匹配进一步使SigLIP-B16在MMVP-VLM上超越GPT-4.1，在16个数据集变体上实现一致改进，在WhatsUp等挑战性数据集上相对增益高达85.7%。

**Conclusion:** 研究揭示了评估指标的系统性偏差问题，提出的方法能够有效释放模型的组合推理潜力，测试时匹配算法具有广泛适用性，即使在无组结构的基准上也能显著提升性能，推动了组合推理研究的前沿发展。
...
#### 🧩 TL;DR
本研究引入了一个可复现的模拟框架来评估多模态环境中生成式代理的安全性、连贯性和信任度，揭示了当前代理在检测多模态矛盾方面的能力有限，且在全局安全对齐方面存在显著不足。

---

#### 📘 Detailed Summary
**Motivation:** 尽管大型语言模型和视觉语言模型使代理能够在丰富环境中自主行动并追求目标，但它们在跨模态安全性、连贯性和信任推理方面的能力仍然有限，这构成了当前研究需要解决的关键问题。

**Method:** 研究提出了一个可复现的模拟框架，配备分层记忆、动态规划、多模态感知能力，并引入了SocialMetrics行为与结构指标套件，用于量化计划修订、不安全到安全转换以及网络中的信息扩散。

**Result:** 实验结果显示代理在多模态矛盾检测方面表现良好，但在全局安全对齐方面仅达到55%的成功率；三个模型（Claude、GPT-4o mini、Qwen-VL）的不安全到安全转换率分别为75%、55%和58%；在多风险场景中GPT-4o mini表现最差（20%），而在局部场景如火灾/高温中Claude表现最佳（98%）；45%的不安全行动在误导性视觉信息下被接受。

**Conclusion:** 研究结果揭示了当前架构在跨模态安全推理方面的关键局限性，特别是代理倾向于过度信任图像信息，这为研究多模态安全性、连贯性和社会动态提供了一个可复现的平台，并指出了未来改进的方向。
...
#### 🧩 TL;DR
本文提出了FinMR，一个高质量、知识密集的多模态数据集，专门用于评估专业分析师级别的金融推理能力，填补了金融领域专业级MLLM评估的空白。

---

#### 📘 Detailed Summary
**Motivation:** 当前多模态大语言模型在金融等专业领域的严格评估受到限制，主要原因是缺乏具有专业级知识强度、详细标注和高级推理复杂度的数据集，这阻碍了对模型专业能力的准确评估。
...
#### 🧩 TL;DR
本文提出了Augur框架，这是首个完全由大语言模型驱动的时序预测方法，通过利用LLM的因果推理能力发现协变量间的有向因果关联，在提升预测准确性的同时提供可解释的变量交互推理。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于大语言模型的时序预测方法存在显著局限性，包括在模型架构中的边缘化角色、依赖粗糙的统计文本提示以及缺乏可解释性，这些问题限制了LLM在时序分析中的潜力发挥。

**Method:** Augur采用两阶段师生架构，首先由强大的教师LLM通过启发式搜索和成对因果检验从时序数据中推断有向因果图，然后轻量级学生代理精炼该图并基于高置信度因果关联进行微调，这些关联被编码为丰富的文本提示用于预测。

**Result:** 在真实世界数据集上进行的广泛实验表明，Augur在25个基线方法中取得了具有竞争力的性能表现，并展现出强大的零样本泛化能力。

**Conclusion:** 该研究证明了LLM因果推理在时序预测中的有效性，不仅提升了预测精度，还提供了透明、可追溯的变量交互推理，为可解释AI时序分析开辟了新途径。
...
#### 🧩 TL;DR
本文提出Chain-of-Trigger Backdoor (CoTri)，一种针对LLM智能体的多步后门攻击方法，能够在保持高攻击成功率的同时增强智能体的良性任务性能，揭示了智能体安全性的严重漏洞。

---

#### 📘 Detailed Summary
**Motivation:** 随着基于大语言模型的智能体在现实应用中的快速部署，其可信赖性问题日益突出，现有后门攻击仅限于单步控制，无法应对智能体长期决策过程中的安全威胁。

**Method:** CoTri采用有序触发序列设计，初始触发后从环境中提取后续触发条件，实现多步操纵以引导智能体偏离原定任务，同时通过训练数据建模环境随机性来增强攻击隐蔽性。

**Result:** 实验结果显示CoTri达到接近完美的攻击成功率(ASR)和接近零的误触发率(FTR)，植入后门反而提升了智能体在良性任务上的性能表现，并在视觉语言模型上验证了该方法的可扩展性。

**Conclusion:** CoTri实现了对智能体的稳定多步控制，同时增强了其固有鲁棒性和任务能力，这种攻击方式更加隐蔽且带来潜在安全风险，突显了智能体安全防护的紧迫需求。
...
#### 🧩 TL;DR
本研究提出了一种轻量级解码器架构，通过动态门控机制实现语言和视觉信息的自适应融合，在认知合理的数据量约束下实现了竞争性的多模态学习性能。该方法在五个基准测试中表现优异，且动态门控机制无需显式监督即可发现可解释的模式。

---

#### 📘 Detailed Summary
**Motivation:** 本研究旨在解决在认知合理数据量约束下训练视觉语言模型时，如何有效整合多模态信息的问题。BabyLM Challenge 2025的视觉赛道限制促使我们重新思考模型如何高效融合语言和视觉线索，特别是在有限视觉信息的情况下最大化其效用。

**Method:** 我们提出了一种轻量级解码器架构，包含三个关键技术：基于token的动态门控机制用于自适应融合语言和视觉线索；特征调制和通道注意力机制以最大化有限视觉信息的效用；以及用于视觉接地的辅助对比学习目标。该架构在严格的计算和数据约束下实现了高效的多模态学习。

**Result:** 在五个基准测试（BLiMP、BLiMP Supplement、EWoK、Winoground和VQA）上的评估显示，我们的方法在性能上达到或超越了多模态基线模型。特别值得注意的是，动态门控机制无需显式监督即可发现可解释的模式：对内容词倾向于使用视觉线索，对功能词则偏好语言线索。

**Conclusion:** 尽管识别出挑战约束带来的局限性，如全局图像嵌入造成的信息瓶颈和数据集分割导致的训练不稳定性，但我们的研究确立了动态门控作为高效多模态学习的强大工具。该方法在严格约束下同时提供了可解释性和性能优势，为资源受限环境下的多模态学习开辟了新途径。
...
#### 🧩 TL;DR
本研究提出了两种简单的微调方法来解决大型多模态模型在顺序学习新技能时的灾难性遗忘问题，通过选择性更新特定网络层来保持模型在目标任务上的强学习能力同时最小化对已有能力的损害。

---

#### 📘 Detailed Summary
**Motivation:** 本研究旨在解决大型多模态模型在顺序微调学习新技能时出现的灾难性遗忘问题，即在学习新任务过程中会显著损害模型在先前任务上的性能表现。

**Method:** 研究提出了两种简单而鲁棒的微调方法：仅更新自注意力投影层，以及仅更新MLP的Gate&Up投影层同时冻结Down投影层，这些选择性更新策略旨在限制输出token分布的漂移。

**Result:** 实验在三个模型家族上进行，涵盖五个目标技能和八个保留基准测试，结果显示所提出的方法在获得强大目标任务性能的同时，能够显著保留模型在保留任务上的表现。

**Conclusion:** 研究发现输出token分布的漂移是导致遗忘的关键因素，通过简单的选择性层更新策略可以有效缓解灾难性遗忘，为大型多模态模型的持续学习提供了实用的解决方案。
