...
### [1] [LP$^{2}$DH: A Locality-Preserving Pixel-Difference Hashing Framework for Dynamic Texture Recognition](https://arxiv.org/abs/2604.15707)
*Ruxin Ding, Jianfeng Ren, Heng Yu, Jiawei Li, Xudong Jiang*

#### 🧩 TL;DR
本文提出了一种局部保持的像素差异哈希（LP²DH）框架，用于解决动态纹理描述子STLBP的高维问题，通过联合编码完整时空邻域中的像素差异，实现了紧凑且具有判别力的二进制特征表示。

---

#### 📘 Detailed Summary
**Motivation:** 时空局部二值模式（STLBP）作为广泛使用的动态纹理描述子存在维度极高的问题，现有方法通常在三个正交平面上提取特征，但牺牲了平面间的相关性，因此需要一种能够有效降低维度同时保持判别力的新方法。

**Method:** 提出的LP²DH框架将像素差异向量（PDVs）转换为紧凑的二进制编码，并引入局部保持嵌入来维持哈希前后PDVs的局部结构，采用Stiefel流形上的梯度下降和曲线搜索策略联合优化哈希矩阵和二进制码，最后通过字典学习将二进制向量编码为码字并生成直方图特征表示。

**Result:** 在三个主要动态纹理识别基准测试中，LP²DH取得了最先进的性能：在UCLA数据集上达到99.80%（优于DT-GoogleNet的98.93%），在DynTex++上达到98.52%（优于HoGF³D的97.63%），在YUPENN上达到96.19%（优于STS的95.00%）。

**Conclusion:** 该研究表明，通过联合编码完整时空邻域的像素差异并保持局部结构，可以显著提升动态纹理识别的性能，LP²DH框架为高维特征的有效压缩和表示提供了新的解决方案，在多个基准测试中验证了其优越性。
...
### [2] [NeuroLip: An Event-driven Spatiotemporal Learning Framework for Cross-Scene Lip-Motion-based Visual Speaker Recognition](https://arxiv.org/abs/2604.15718)
*Junguang Yao, Wenye Liu, Stjepan Picek, Yue Zheng*

#### 🧩 TL;DR
本文提出NeuroLip，一种基于事件相机的唇部运动识别框架，通过捕捉精细的唇部动态行为特征，在跨场景协议下实现了鲁棒的视觉说话人识别。该方法结合了时间感知体素编码、结构感知空间增强器和极性一致性正则化，并在新构建的DVSpeaker数据集上验证了其优越性能。

---

#### 📘 Detailed Summary
**Motivation:** 传统基于外观的视觉说话人识别方法受环境变化影响较大，而唇部运动编码了主体特定的行为动态，具有内在稳定性。然而，传统帧式相机难以捕捉精细的唇部动态，存在运动模糊和动态范围低的限制。本研究旨在利用事件相机解决这些传感限制，在严格的跨场景协议下实现鲁棒的唇部运动识别。

**Method:** NeuroLip框架包含三个核心组件：1）具有自适应事件加权的时间感知体素编码模块，用于有效表示事件数据；2）结构感知空间增强器，通过抑制噪声同时保留垂直结构运动信息来放大判别性行为模式；3）极性一致性正则化机制，保留事件极性编码的运动方向线索。该框架在单次受控条件下训练，但必须在未见过的视角和光照条件下进行识别。

**Result:** 在构建的DVSpeaker数据集（包含50个主体在四种不同视角和光照场景下的记录）上的实验表明，NeuroLip实现了近乎完美的匹配场景准确率，并在跨场景泛化方面表现优异。在未见过的视角上达到超过71%的准确率，在低光照条件下接近76%，比现有代表性方法至少高出8.54%。

**Conclusion:** NeuroLip证明了事件相机在捕捉精细唇部动态方面的优势，为视觉说话人识别提供了鲁棒的解决方案。该方法在严格跨场景协议下的成功表明，基于行为动态的识别方法比传统外观依赖方法更具环境稳定性。公开的数据集和代码为未来事件视觉研究提供了宝贵资源。
...
### [3] [Neuromorphic Parameter Estimation for Power Converter Health Monitoring Using Spiking Neural Networks](https://arxiv.org/abs/2604.15714)
*Hyeongmeen Baik, Hamed Poursiami, Maryam Parsa, Jinia Roy*

#### 🧩 TL;DR
本文提出了一种用于电力转换器健康监测的节能边缘推理方法，通过将脉冲神经网络的时间处理与物理约束解耦，实现了在亚毫瓦级功耗下的准确参数估计和故障检测。

---

#### 📘 Detailed Summary
**Motivation:** 持续运行的转换器健康监测需要亚毫瓦级的边缘推理能力，而基于GPU的物理信息神经网络无法在此功耗范围内运行，这限制了实时监控系统的部署可行性。

**Method:** 该方法将脉冲时间处理与物理约束分离：采用三层泄漏积分发放脉冲神经网络估计无源元件参数，同时通过可微分ODE求解器提供物理一致性训练，将ODE物理损失与展开的脉冲循环解耦。

**Result:** 在EMI干扰的同步降压转换器基准测试中，SNN将集总电阻误差从25.8%降低至10.2%，处于无源元件±10%制造公差范围内，在神经形态硬件上预计实现约270倍的能量减少，93%的脉冲稀疏性使其适合在Intel Loihi 2或BrainChip Akida上持续部署。

**Conclusion:** 该架构通过分离时间处理与物理约束，为低功耗边缘设备上的实时健康监测提供了可行方案，持续的膜电位状态支持退化跟踪和事件驱动的故障检测，在突发故障时通过+5.5个百分点的脉冲率跳变实现故障识别。
...
### [4] [Combining Convolution and Delay Learning in Recurrent Spiking Neural Networks](https://arxiv.org/abs/2604.15997)
*Lúcio Folly Sanches Zebendo, Eleonora Cicciarella, Michele Rossi*

#### 🧩 TL;DR
该研究提出了一种结合卷积循环连接与延迟学习机制的尖峰神经网络架构，在音频分类任务上实现了参数数量减少99%和推理速度提升52倍的同时保持了原有精度。

---

#### 📘 Detailed Summary
**Motivation:** 尖峰神经网络在资源受限的边缘系统中作为传统人工神经网络的替代方案日益受到关注，但现有方法在参数效率和推理速度方面仍有改进空间，特别是结合循环连接与延迟学习机制的架构优化潜力尚未充分挖掘。

**Method:** 研究扩展了先前提出的DelRec延迟学习机制，将其与卷积循环连接相结合，构建了一种新型的尖峰神经网络架构，其中轴突延迟与网络参数在运行时共同学习，实现了更紧凑的网络结构设计。

**Result:** 在音频分类任务上的实验表明，所提方法在保持DelRec原有精度的同时，将循环参数数量减少了约99%，推理速度提升了52倍，显著降低了内存占用并大幅加速了推理过程。

**Conclusion:** 该研究表明卷积循环连接与延迟学习机制的结合能够显著提升尖峰神经网络的参数效率和推理速度，为资源受限的边缘设备部署提供了更优的解决方案，同时保持了模型的表达能力。
