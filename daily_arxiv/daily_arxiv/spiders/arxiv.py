...
class ArxivSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for paper in response.css("dl dt"):
            ...
            if not paper_dd:
                continue
            title_parts = paper_dd.css("div.list-title.mathjax ::text").getall()
            title = " ".join(part.strip() for part in title_parts).replace("Title:", "").strip()

            abstract_parts = paper_dd.css("p.mathjax::text").getall()
            abstract = " ".join(part.strip() for part in abstract_parts).replace("Abstract:", "").strip()

            if self.keywords:
                searchable_text = f"{title} {abstract}".lower()
                if not any(keyword in searchable_text for keyword in self.keywords):
                    self.logger.debug(
                        "Skipped paper %s - no keyword match in title/abstract", arxiv_id
                    )
                    continue
